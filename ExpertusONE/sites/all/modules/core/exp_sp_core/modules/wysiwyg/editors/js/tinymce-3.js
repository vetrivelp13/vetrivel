...
Drupal.wysiwyg.editor.attach.tinymce = function(context, params, settings) {
  ...
  // Expertus: Start - implement keyboard navigation in form with tinymce editor - ticket #0019466
  // Solution source:  http://www.pixelastic.com/blog/159:tabindexing-and-inserting-tabs-with-tinymce
  ed.onKeyDown.add(function(editor, event) {
    // We only listen for the tab key
    if (event.keyCode!=9) return;

    var tabindex = $('#'+editor.editorId+'_ifr').attr('tabindex');
    //console.log('tinymce-3.js : ed.onKeyDown.add() : iframe tabindex = ' + tabindex);
    // We get all the tabindexed elements of the page
    var inputs = [];
    $(':input[tabindex]').each(function() {
      //console.log($(this).attr('tabindex') + ' - ' + $(this).attr('id'));
      inputs[$(this).attr('tabindex')] = this;
    });
    
    // Integer sort the tab indexes
    var tidx = [];
    for(var key in inputs) {
      if(inputs.hasOwnProperty(key)) { //to be safe
        tidx.push(key);
      }
    }
    tidx.sort(function (a, b) { return parseInt(a) - parseInt(b); });
    //console.log('array sorted');
    
    // For Shift-Tab navigate backward, for Tab navigate forward
    if (event.shiftKey) {
      for (var n = tidx.length - 1; n >= 0; n--) {
        //console.log(tidx[n]);
        if (parseInt(tidx[n]) >= parseInt(tabindex)) continue;
        //console.log('Smaller element found ' + $(inputs[tidx[n]]).attr('id'));
        if (!$(inputs[tidx[n]]).is(':visible')) continue;
        if ( $(inputs[tidx[n]]).is(':disabled')) continue;
        //console.log('setting focus on ' + $(inputs[tidx[n]]).attr('id'));
        inputs[tidx[n]].focus();
        break;
      }
    }
    else {
      for (var n = 0; n < tidx.length; n++) {
        //console.log(tidx[n]);
        if (parseInt(tidx[n]) <= parseInt(tabindex)) continue;
        //console.log('Greater element found ' + $(inputs[tidx[n]]).attr('id'));
        if (!$(inputs[tidx[n]]).is(':visible')) continue;
        if ( $(inputs[tidx[n]]).is(':disabled')) continue;
        //console.log('setting focus on ' + $(inputs[tidx[n]]).attr('id'));
        inputs[tidx[n]].focus();
  ...
  // Expertus: End - implement keyboard navigation in form with tinymce editor - ticket #0019466
  ...
  // Expertus: Start - implement keyboard navigation in form with tinymce editor - ticket #0019466
  // Solution source:  http://www.pixelastic.com/blog/159:tabindexing-and-inserting-tabs-with-tinymce  
  //console.log('tinymce-3.js :Drupal.wysiwyg.editor.attach.tinymce() : may swap tab index');
  var editorId = ed.editorId;
  var textarea = $('#' + editorId);
  var textareaTabIndex = textarea.attr('tabindex');
  var iframeId = '#' + editorId + '_ifr';
  //console.log(textareaTabIndex);
  if (typeof textareaTabIndex !== 'undefined' && textareaTabIndex !== false && textareaTabIndex !== 0) {
    //console.log('tinymce-3.js :Drupal.wysiwyg.editor.attach.tinymce() : swapping tab index');
    textarea.removeAttr('tabindex');
    
    var setIframeTabIndex = function() {
      if (jQuery(iframeId).length <= 0) {
        //console.log('setIframeTabIndex() : iframe is not');
        setTimeout(setIframeTabIndex, 200);
        return;
      }
      //console.log('setIframeTabIndex() : setting tab index of iframe ' + iframeId + '  ' + textareaTabIndex + ' ' + jQuery(iframeId).length);
      jQuery(iframeId).attr('tabindex', textareaTabIndex);      
    };
    $(document).ready(function() {
        setTimeout(setIframeTabIndex, 500);
    });
  }
  // Expertus: End - implement keyboard navigation in form with tinymce editor - ticket #0019466
};
