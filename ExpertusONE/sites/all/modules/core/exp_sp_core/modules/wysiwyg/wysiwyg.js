  ...
  attach: function (context, settings) {
    ...
    $('.wysiwyg', context).once('wysiwyg', function () {
      ...
      if ($this.is(':input')) {
        ...
        // Temporary fix for ticket #18723. Ensure editor is displaying after attach.
        $(document).ready( function () { setTimeout(function () {
                            jQuery('#' + params.field + '_parent').show();
                            // If error class is set for field, show a red border around the editor
                            if (jQuery('#' + params.field).hasClass('error')) {
                            		//$('table#' + params.field + '_tbl').css('border', '1px solid red');
                            		$('table#' + params.field + '_tbl .mceIframeContainer').css('border-color', '#f00');
                            		$('table#' + params.field + '_tbl .mceToolbar').css('border-color', '#f00');
                            		$('table#' + params.field + '_tbl .mceLayout').css('border-color', '#f00'); 
                            }
                          }, 30);
        });
      }
