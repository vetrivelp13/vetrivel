...
if (theme_get_setting('expertusone_rebuild_registry') && !defined('MAINTENANCE_MODE')) {
  // Rebuild .info data.
 ...
 */
function expertusone_theme(&$existing, $type, $theme, $path) {
  include_once './' . drupal_get_path('theme', 'expertusone') . '/expertusone-internals/template.theme-registry.inc';
  return _expertusone_theme($existing, $type, $theme, $path);
}
 ...
 */
function expertusone_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('expertusone_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('expertusone_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('expertusone_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('expertusone_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('expertusone_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function expertusone_menu_local_tasks(&$variables) {
  $output = '';
 ...
 */
function expertusone_preprocess_menu_local_task(&$variables) {
 ...
 */
function expertusone_add_conditional_styles() {
 ...
 */
function expertusone_preprocess_html(&$variables, $hook) {
  // If the user is silly and enables expertusone as the theme, add some styles.
  if ($GLOBALS['theme'] == 'expertusone') {
    include_once './' . drupal_get_path('theme', 'expertusone') . '/expertusone-internals/template.expertusone.inc';
    _expertusone_preprocess_html($variables, $hook);
  }
  elseif (!module_exists('conditional_styles')) {
    expertusone_add_conditional_styles();
  }
  ...
  if (theme_get_setting('expertusone_wireframes')) {
  ...
  $variables['jump_link_target'] = theme_get_setting('expertusone_jump_link_target');
}
 ...
 */
function expertusone_preprocess_page(&$variables, $hook) {
  ...
  expertusone_removetab(array('Log in','Request new password','Create new account'),$variables);
}
 ...
 *   An array of variables to pass to the theme template.
 */
function expertusone_removetab($label, &$vars) {
 ...
 */
function expertusone_preprocess_maintenance_page(&$variables, $hook) {
 ...
 */
function expertusone_preprocess_node(&$variables, $hook) {
 ...
 */
function expertusone_preprocess_comment(&$variables, $hook) {
 ...
 */
function expertusone_preprocess_region(&$variables, $hook) {
 ...
 */
function expertusone_preprocess_block(&$variables, $hook) {
  // Classes describing the position of the block within the region.
  if ($variables['block_id'] == 1) {
    $variables['classes_array'][] = 'first';
  }
  // The last_in_region property is set in expertusone_page_alter().
 ...
 *   The name of the template being rendered ("block" in this case.)
 */
function expertusone_process_block(&$variables, $hook) {
 ...
 */
function expertusone_page_alter(&$page) {
...
function expertusone_menu_link(array $variables) {
 ...
 * @ingroup themeable
 */
function expertusone_preprocess_uc_cart_checkout_exp_paymethod(&$variables) {
...
function expertusone_uc_cart_checkout_review($variables) {
           ...
           '<div id="d-cart-review-wrapper">' .
             '<table style="width:100%">' .
               ...
               //'<tr>' .
                 ...
                 '<div class="common-style-to-review-order">' . $output_pane_list[2] . '</div>' .
               //'</tr>' .
  						 //'<tr>' .
  						   '<div class="common-style-to-review-order">'.$output_pane_list[1].'</div>' .
  						 //'</tr>' .				
  						 //'<tr>' .
  						 ...
  						 //'</tr>' .
  						 //'<tr>' .
  						   '<div class="common-style-to-review-order order-comment-col">'.$output_pane_list[4]. '</div>' .
  						 //'</tr>' .
  					//'</table>' .
  			  '</div>' .
 ...
 *     of that type.
 */
function expertusone_status_messages($variables) {
  
  expDebug::dPrint('template.php : expertusone_status_messages() : $variables = ' . print_r($variables, true),4);	
  ...
  foreach (drupal_get_messages($display) as $type => $messages) {
    ...
    if ($nonBlankMsgCount > 0) {
      ...
      $output .= '<img onload="try{$(\'body\').data(\'learningcore\').showHideMultipleLi();}catch(e){}" style="display:none;" src="sites/all/themes/core/expertusone/expertusone-internals/images/close.png" height="0" width="0" />';
    }
  }
  return $output;
}
 ...
 * @ingroup themeable
 */
function expertusone_multiselect($variables) {
  expDebug::dPrint('template.php : expertusone_multiselect() : $variables = ' . print_r($variables, true));
  
  $element = $variables['element'];
  expDebug::dPrint('template.php : expertusone_multiselect() : $element = ' . print_r($element, true));
  ...
  expDebug::dPrint('template.php : expertusone_multiselect() : after element_set_attributes() $element = ' . print_r($element, true));
  
  _form_set_class($element, array('form-multiselect'));
  expDebug::dPrint('template.php : expertusone_multiselect() : after _form_set_class() $element = ' . print_r($element, true));
  ...
  expDebug::dPrint('template.php : expertusone_multiselect() : $widget = ' . print_r($widget, true));
  ...
  $renderedElem = $widget['prefix_pre'] .
                  ...
                  '</div>' .
                '</div>';
  expDebug::dPrint('template.php : expertusone_multiselect() : $renderedElem = ' . print_r($renderedElem, true));                     
  return $renderedElem;
}
