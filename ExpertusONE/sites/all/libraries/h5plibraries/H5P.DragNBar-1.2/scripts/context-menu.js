...
H5P.DragNBarContextMenu = (function ($, EventDispatcher) {
  ...
  function ContextMenu($container, DragNBarElement, hasCoordinates, disableResize) {
     ...
     */
    this.buttons = [
      {name: 'Edit',label:window.parent.Drupal.t("LBL063")},// label: H5PEditor.t('H5P.DragNBar', 'editLabel')},
      {name: 'BringToFront', label: window.parent.Drupal.t("LBL3205")},
      {name: 'Remove', label: window.parent.Drupal.t("LBL082")}  //H5PEditor.t('H5P.DragNBar', 'removeLabel')}
    ];
  ...
  ContextMenu.prototype.updateCoordinates = function (left, top, x, y) {
    ...
    if($(".h5p-interactivevideo-editor").size() > 0)
    {
     ...
     }else if($(".h5p-coursepresentation-editor").size() > 0){
     	if(top < 19)
    	{
    		if($(".e1-tip").size() > 0){
    		   this.$contextMenu.css({
  ...
  ContextMenu.prototype.addTransform = function (enableTransform) {
    var self = this;
    var transformButtonObject = {name: 'Transform', label:window.parent.Drupal.t("LBL3206")}; // H5PEditor.t('H5P.DragNBar', 'transformLabel')};
