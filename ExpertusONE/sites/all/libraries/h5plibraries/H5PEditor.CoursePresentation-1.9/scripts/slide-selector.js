  ...
  function SlideSelector(cpEditor, $slides, globalFields, slideFields, params) {
    ...
    var sortSlide = function (dir) {
      ...
      if ((currentSlide + dir >= 0) && (currentSlide + dir < $slides.children().length)) {
    	 ...
    	 if($(".e1_h5p_error1").find("#message-container").size() != 0)
    	  $(".e1_h5p_error1").find("#message-container").css({"visibility":"hidden"});  
    	...
    	var htm  =  '<a id="control-close" class="qtip-close-button" onclick="closeH5PIcon(this)"></a>';
    		htm +=  '<div class="e1_h5p_error1" style="height:auto;"></div>';
    		$(".h5p-course-presentation .h5p-presentation-wrapper").append(htm);
    	//  alert("Already Sorted");
    	  
    	  var msg = window.parent.Drupal.t('Already Sorted');
    	  $('.e1_h5p_error1').html("");
    	  $('.e1_h5p_error1').show();
    		var data = '<div id="show_expertus_message" style="margin-top:3px;"><div><div id="message-container" style="visibility: visible;"><div class="messages error">'+msg+'<div class="msg-close-btn" onclick=\"$(\'.e1_h5p_error1\').hide();\"></div></div></div><img  style=\"display:none;\"  src=\"sites/all/themes/core/expertusoneV2/expertusone-internals/images/close.png\" height=\"0\" width=\"0\"></div></div>';
    	$(".e1_h5p_error1").html(data);
      }
    };
    ...
    var changeSlideType = function ($content) {
	  ...
	  //H5PCUSTOMIZE - get the current slide no and invoke click method of "1.color fill background" but 0 is upload image
	  var currentSlideNo = H5P.jQuery(".h5p-footer-slide-count-current").html();
	  // fix for #84374 - sometimes click event not available. added try catch
	  try
	  {
		 H5P.jQuery("input[name='h5p-radio-selector-"+currentSlideNo+"']")[1].click();
	  }catch(e){
