...
var fc = $newJquery.fullCalendar = { version: "2.4.0" };
var fcViews = fc.views = {};


$newJquery.fn.fullCalendar = function(options) {
	var args = Array.prototype.slice.call(arguments, 1); // for a possible method call
	var res = this; // what this function will return (this jQuery object by default)

	this.each(function(i, _element) { // loop each DOM element involved
		var element = $newJquery(_element);
		var calendar = element.data('fullCalendar'); // get the existing calendar object (if any)
		var singleRes; // the returned value of this single method call

		// a method call
		if (typeof options === 'string') {
			if (calendar && $newJquery.isFunction(calendar[options])) {
...
function massageOverrides(input) {
	...
	$newJquery.each(input, function(name, val) {
		if (name != 'views') {

			// could the value be a legacy View-Option-Hash?
			if (
				$newJquery.isPlainObject(val) &&
				!/(time|duration|interval)$/i.test(name) && // exclude duration options. might be given as objects
				$newJquery.inArray(name, complexOptions) == -1 // complex options aren't allowed to be View-Option-Hashes
			) {
				subObj = null;

				// iterate through the properties of this possible View-Option-Hash value
				$newJquery.each(val, function(subName, subVal) {
...
function disableCursor() {
	$newJquery('body').addClass('fc-not-allowed');
}


// Returns the mouse cursor to its original look
function enableCursor() {
	$newJquery('body').removeClass('fc-not-allowed');
}
...
function distributeHeight(els, availableHeight, shouldRedistribute) {
	...
	els.each(function(i, el) {
		...
		var naturalOffset = $newJquery(el).outerHeight(true);

		if (naturalOffset < minOffset) {
			flexEls.push(el);
			flexOffsets.push(naturalOffset);
			flexHeights.push($newJquery(el).height());
		}
		else {
	...
	$newJquery(flexEls).each(function(i, el) {
...
function matchCellWidths(els) {
	...
	els.find('> *').each(function(i, innerEl) {
		var innerWidth = $newJquery(innerEl).outerWidth();
...
function getScrollParent(el) {
	var position = el.css('position'),
		scrollParent = el.parents().filter(function() {
			var parent = $newJquery(this);
			return (/(auto|scroll)/).test(
				parent.css('overflow') + parent.css('overflow-y') + parent.css('overflow-x')
			);
		}).eq(0);

	return position === 'fixed' || !scrollParent.length ? $newJquery(el[0].ownerDocument || document) : scrollParent;
}
...
function parseFieldSpecs(input) {
	...
	else if ($newJquery.isArray(input)) {
		tokens = input;
	}
...
function flexibleCompare(a, b) {
	...
	if ($newJquery.type(a) === 'string' || $newJquery.type(b) === 'string') {
...
function isAtomic(val) {
	return /undefined|null|boolean|number|string/.test($newJquery.type(val));
}


function applyAll(functions, thisObj, args) {
	if ($newJquery.isFunction(functions)) {
		functions = [ functions ];
	}
	if (functions) {
		var i;
...
function cssToStr(cssProps) {
	...
	$newJquery.each(cssProps, function(name, val) {
		if (val != null) {
...
var oldMomentProto = $newJquery.extend({}, newMomentProto); // copy of original moment methods
...
var Emitter = fc.Emitter = Class.extend({
	...
	getCallbacks: function(name) {
		...
		if (!callbacks) {
			callbacks = this.callbackHash[name] = $newJquery.Callbacks();
		}
...
var Popover = Class.extend({
	...
	render: function() {
		...
		// Admin calendar change - drop internal elements inside more-popup
		var popUpCnt = $newJquery('<div id="admincalendarviewmorepopup" class="'+boxposition+'">'
			+ '<div class="dropdownadd-dd-list-arrow '+boxposition+'"></div>'
			+ '<a class="qtip-close-button account-close-popup fc-close"></a>'
		...
		this.el = $newJquery('<div class="fc-popover" />')
		...
		$newJquery("#admincalendarviewmorepopup .rows").html(options.content[1]);
		attachEvents('#admincalendarviewmorepopup .fc-title a');
		$("#admincalendarviewmorepopup .rows").jScrollPane();
		vtip();
		...
		if (options.autoHide) {
			$newJquery(document).on('mousedown', this.documentMousedownProxy = proxy(this, 'documentMousedown'));
		}
	},
	...
	documentMousedown: function(ev) {
		...
		if (this.el && !$newJquery(ev.target).closest(this.el).length) {
			this.hide();
		}
	},
	...
	removeElement: function() {
		...
		$newJquery(document).off('mousedown', this.documentMousedownProxy);
	},
	...
	getCell: function(x, y) {
		...
		if (this.inBounds(x, y)) {
			...
			if (hitRow !== null && hitCol !== null) {
				...
				$newJquery.extend(cell, rowCoords[hitRow], colCoords[hitCol]);
	...
	startListening: function(ev) {
		...
		if (!this.isListening) {
			...
			if (ev && this.options.scroll) {
				scrollParent = getScrollParent($newJquery(ev.target));
...
var MouseFollower = Class.extend({
	...
	constructor: function(sourceEl, options) {
		...
		this.parentEl = options.parentEl ? $newJquery(options.parentEl) : sourceEl.parent(); // default to sourceEl's parent
	},
	...
	start: function(ev) {
		if (!this.isFollowing) {
			...
			$newJquery(document).on('mousemove', this.mousemoveProxy = proxy(this, 'mousemove'));
		}
	},
	...
	stop: function(shouldRevert, callback) {
		...
		if (this.isFollowing && !this.isAnimating) { // disallow more than one stop animation at a time
			...
			$newJquery(document).off('mousemove', this.mousemoveProxy);
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	getCell: function(row, col) {
		...
		$newJquery.extend(cell, this.getRowData(row), this.getColData(col));
		$newJquery.extend(cell, this.computeCellRange(cell));
	...
	setElement: function(el) {
		...
		el.on('mousedown', function(ev) {
			if (
				!$newJquery(ev.target).is('.fc-event-container *, .fc-more') && // not an an event element, or "more.." link
				!$newJquery(ev.target).closest('.fc-popover').length // not on a popover (like the "more.." events one)
			) {
	...
	bindGlobalHandlers: function() {
		$newJquery(document).on('dragstart sortstart', this.externalDragStartProxy); // jqui
	},


	// Unbinds DOM handlers from elements that reside outside the grid
	unbindGlobalHandlers: function() {
		$newJquery(document).off('dragstart sortstart', this.externalDragStartProxy); // jqui
	},
	...
	renderFillSegEls: function(type, segs) {
		...
		if (segs.length) {
			...
			$newJquery(html).each(function(i, node) {
				var seg = segs[i];
				var el = $newJquery(node);
				...
				if (el) { // custom filters did not cancel the render
					el = $newJquery(el); // allow custom filter to return raw DOM node
	...
	headCellHtml: function(cell) {
		var view = this.view;
		var date = cell.start;
		var cellGridClass = "fc-grid-" + cell.row + cell.col; // Admin calendar change - Thamil 
		return '' +
			'<th class="fc-day-header ' + view.widgetHeaderClass + ' ' + cellGridClass + ' fc-' + dayIDs[date.day()] + '">' +
...
Grid.mixin({
	...
	renderFgSegEls: function(segs, disableResizing) {
		...
		if (segs.length) { // don't build an empty html string
			...
			$newJquery(html).each(function(i, node) {
				var seg = segs[i];
				var el = view.resolveEventEl(seg.event, $newJquery(node));
		...
		$newJquery.each(
			{
				...
				mousedown: function(seg, ev) {
					if ($newJquery(ev.target).is('.fc-resizer') && view.isEventResizable(seg.event)) {
						_this.segResizeMousedown(seg, ev, $newJquery(ev.target).is('.fc-start-resizer'));
					}
			...
			function(name, func) {
				...
				_this.el.on(name, '.fc-event-container > *', function(ev) {
					var seg = $newJquery(this).data('fc-seg'); // grab segment data. put there by View::renderEvents
	...
	externalDragStart: function(ev, ui) {
		...
		if (view.opt('droppable')) { // only listen if this setting is on
			el = $newJquery((ui ? ui.item : null) || ev.target);

			// Test that the dragged element passes the dropAccept selector or filter function.
			// FYI, the default is "*" (matches all)
			accept = view.opt('dropAccept');
			if ($newJquery.isFunction(accept) ? accept.call(el[0], el) : el.is(accept)) {
	...
	eventsToRanges: function(events) {
		...
		$newJquery.each(eventsById, function(id, eventGroup) {
...
function getDraggedElMeta(el) {
	...
	if (eventProps) {
		if (typeof eventProps === 'object') {
			eventProps = $newJquery.extend({}, eventProps); // make a copy
		}
...
var DayGrid = Grid.extend({
		...
		return '' +
			'<div class="' + classes.join(' ') + '">' +
				'<div class="fc-bg">' +
					...
					'</table>' +
				'</div>' +
				'<div id="fc-content-skeleton"><div class="fc-content-skeleton">' +
					'<table>' +
						(this.numbersVisible ?
							'<thead>' +
	...
	renderHelper: function(event, sourceSeg) {
		...
		this.rowEls.each(function(row, rowNode) {
			var rowEl = $newJquery(rowNode); // the .fc-row
			var skeletonEl = $newJquery('<div class="fc-helper-skeleton"><table/></div>'); // will be absolutely positioned
			var skeletonTop;
		...
		this.helperEls = $newJquery(helperNodes); // array -> jQuery set
	},
	...
	renderFill: function(type, segs, className) {
		...
		this.elsByFill[type] = $newJquery(nodes);
...
DayGrid.mixin({
	...
	renderBgSegs: function(segs) {
		...
		var allDaySegs = $newJquery.grep(segs, function(seg) {
	...
	renderFgSegs: function(segs) {
		...
		this.rowEls.each(function(i, rowNode) {
			$newJquery(rowNode).find('.fc-content-skeleton > table').append(
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			if (levelSegs) {
				for (j = 0; j < levelSegs.length; j++) { // iterate through segments in level
					...
					td = $newJquery('<td class="fc-event-container"/>').append(seg.el);
...
DayGrid.mixin({
	...
	computeRowLevelLimit: function(row) {
		...
		function iterInnerHeights(i, childNode) {
			trHeight = Math.max(trHeight, $newJquery(childNode).outerHeight());
		}
		...
		function emptyCellsUntil(endCol) { // goes from current `col` to `endCol`
			while (col < endCol) {
				...
				if (segsBelow.length) {
					...
					moreWrap = $newJquery('<div/>').append(moreLink);
					td.append(moreWrap);
			...
			for (i = 0; i < levelSegs.length; i++) {
				...
				if (totalSegsBelow) { // do we need to replace this segment with one or many "more" links?
					...
					for (j = 0; j < colSegsBelow.length; j++) {
						moreTd = $newJquery('<td class="fc-more-cell"/>').attr('rowspan', rowspan);
					...
					td.addClass('fc-limited').after($newJquery(segMoreNodes)); // hide original <td> and inject replacements
					limitedNodes.push(td[0]);
				}
			}

			emptyCellsUntil(this.colCnt); // finish off the level
			rowStruct.moreEls = $newJquery(moreNodes); // for easy undoing later
			rowStruct.limitedEls = $newJquery(limitedNodes); // for easy undoing later
		}
	},
	...
	renderMoreLink: function(cell, hiddenSegs) {
		var cellIdClass = "fc-more-" + cell.row + cell.col // Admin calendar change - Thamil
		var _this = this;
		var view = this.view;

		return $newJquery('<a class="fc-more '+cellIdClass+'"/>') // Admin calendar change - Thamil
			.text(
				//this.getMoreLinkText(hiddenSegs.length)
					this.getMoreLinkText('')
			)
			.on('click', function(ev) {
				var clickOption = view.opt('eventLimitClick');
				var date = cell.start;
				var moreEl = $newJquery(this);
	...
	showSegPopover: function(cell, moreLink, segs) {
		...
		options = {
			...
			parentEl: this.el,
			top: moreWrap.offset().top, // Admin calendar change - Thamil
	...
	resliceDaySegs: function(segs, dayDate) {
		...
		var events = $newJquery.map(segs, function(seg) {
			return seg.event;
		});
...
var TimeGrid = Grid.extend({
	...
	computeSlatTops: function() {
		...
		this.slatEls.each(function(i, node) {
			top = $newJquery(node).position().top;
			tops.push(top);
		});
	...
	renderHelper: function(event, sourceSeg) {
		...
		this.helperEl = $newJquery('<div class="fc-helper-skeleton"/>')
			.append(tableEl)
				.appendTo(this.el);
	},
	...
	renderFill: function(type, segs, className) {
		...
		if (segs.length) {
			...
			for (col = 0; col < segCols.length; col++) {
				colSegs = segCols[col];
				tdEl = $newJquery('<td/>').appendTo(trEl);

				if (colSegs.length) {
					containerEl = $newJquery('<div class="fc-' + className + '-container"/>').appendTo(tdEl);
...
TimeGrid.mixin({
	...
	renderFgSegs: function(segs) {
		...
		this.el.append(
			this.eventSkeletonEl = $newJquery('<div class="fc-content-skeleton"/>')
	...
	renderSegTable: function(segs) {
		var tableEl = $newJquery('<table><tr/></table>');
		...
		for (col = 0; col < segCols.length; col++) { // iterate each column grouping
			...
			containerEl = $newJquery('<div class="fc-event-container"/>');
			...
			trEl.append($newJquery('<td/>').append(containerEl));
		}
	...
	generateSegPositionCss: function(seg) {
        ...
        if(seg.level>0){        	
        	props.width = 88;
         	props.left = props.width*seg.level ;
        	props.left = parseInt(props.left)+ 'px';
        }
...
var View = fc.View = Class.extend({
	...
	display: function(date) {
		...
		return this.clear().then(function() { // clear the content first (async)
			return (
				_this.displaying =
					$newJquery.when(_this.displayView(date)) // displayView might return a promise
						.then(function() {
	...
	clear: function() {
		...
		if (displaying) { // previously displayed, or in the process of being displayed?
			...
			return $newJquery.when(); // an immediately-resolved promise
		}
	},
	...
	bindGlobalHandlers: function() {
		$newJquery(document).on('mousedown', this.documentMousedownProxy);
	},
	...
	reportExternalDrop: function(meta, dropLocation, el, ev, ui) {
		...
		if (eventProps) {
			eventInput = $newJquery.extend({}, eventProps, dropLocation);
	...
	documentMousedown: function(ev) {
		...
		if (this.isSelected && this.opt('unselectAuto') && isPrimaryMouseButton(ev)) {
			...
			if (!ignore || !$newJquery(ev.target).closest(ignore).length) {
				this.unselect(ev);
			}
		}
	},
	...
	initHiddenDays: function() {
		...
		for (i = 0; i < 7; i++) {
			if (
				!(isHiddenDayHash[i] = $newJquery.inArray(i, hiddenDays) !== -1)
			) {
				dayCnt++;
			}
		}
	...
	computeDayRange: function(range) {
		...
		if (end) {
			...
			//console.log(range.event.end);
			//console.log(startDay);
			var nexthreshold = false;
			if(range.event != undefined && range.event.type =='cre_sys_obt_anm')
				nexthreshold = validatenexthreshold(range);
			if (endTimeMS && endTimeMS >= this.nextDayThreshold || nexthreshold) { // Admin Calendar change - Thamil
				endDay.add(1, 'days');
			}
		}
...
var Calendar = fc.Calendar = Class.extend({
	...
	getUnitViewSpec: function(unit) {
		...
		if ($newJquery.inArray(unit, intervalUnits) != -1) {

			// put views that have buttons first. there will be duplicates, but oh well
			viewTypes = this.header.getViewsWithButtons();
			$newJquery.each(fc.views, function(viewType) { // all views
...
function Calendar_constructor(element, overrides) {
	...
	function initialRender() {
		...
		content = $newJquery("<div class='fc-view-container'/>").prependTo(element);
		...
		if (options.handleWindowResize) {
			...
			$newJquery(window).resize(windowResizeProxy);
		}
	}
	...
	function destroy() {
		...
		if (windowResizeProxy) {
			$newJquery(window).unbind('resize', windowResizeProxy);
		}
	}
	...
	function renderView(viewType) {
		...
		if (!currentView && viewType) {
			...
			currentView.setElement(
				$newJquery("<div class='fc-view fc-" + viewType + "-view' />").appendTo(content)
			);
...
fc.datepickerLang = function(langCode, dpLangCode, dpOptions) {
	...
	$newJquery.each(dpComputableOptions, function(name, func) {
		fcOptions[name] = func(dpOptions);
	});

	// is jQuery UI Datepicker is on the page?
	if ($newJquery.datepicker) {

		// Register the language data.
		// FullCalendar and MomentJS use language codes like "pt-br" but Datepicker
		// does it like "pt-BR" or if it doesn't have the language, maybe just "pt".
		// Make an alias so the language can be referenced either way.
		$newJquery.datepicker.regional[dpLangCode] =
			$newJquery.datepicker.regional[langCode] = // alias
				dpOptions;

		// Alias 'en' to the default language data. Do this every time.
		$newJquery.datepicker.regional.en = $newJquery.datepicker.regional[''];

		// Set as Datepicker's global defaults.
		$newJquery.datepicker.setDefaults(dpOptions);
	}
};
...
fc.lang = function(langCode, newFcOptions) {
	...
	$newJquery.each(momComputableOptions, function(name, func) {
...
function populateInstanceComputableOptions(options) {
	$newJquery.each(instanceComputableOptions, function(name, func) {
...
function Header(calendar, options) {
	...
	function render() {
		...
		if (sections) {
			el = $newJquery("<div class='fc-toolbar'/>")
	...
	function renderSection(position) {
		var sectionEl = $newJquery('<div class="fc-' + position + '"/>');
		var buttonStr = options.header[position];

		if (buttonStr) {
			$newJquery.each(buttonStr.split(' '), function(i) {
				var groupChildren = $newJquery();
				var isOnlyButtons = true;
				var groupEl;

				$newJquery.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
						groupChildren = groupChildren.add($newJquery('<h2>&nbsp;</h2>')); // we always want it to take up height
						isOnlyButtons = false;
					}
					else {
						...
						if (buttonClick) {
							...
							button = $newJquery( // type="button" so that it doesn't submit a form
...
function EventManager(options) { // assumed to be a calendar
	...
	function fetchEventSource(source, fetchID) {
		_fetchEventSource(source, function(eventInputs) {
			var isArraySource = $newJquery.isArray(source.events);
			var i, eventInput;
			var abstractEvent;
	...
	function _fetchEventSource(source, callback) {
		...
		if (events) {
			if ($newJquery.isFunction(events)) {
				t.pushLoading();
				events.call(
					t, // this, the Calendar object
					rangeStart.clone(),
			...
			else if ($newJquery.isArray(events)) {
				callback(events);
			}
			else {
				callback();
			...
			if (url) {
				...
				// retrieve any outbound GET/POST $newJquery.ajax data from the options
				var customData;
				if ($newJquery.isFunction(source.data)) {
				...
				var data = $newJquery.extend({}, customData || {});
				...
				t.pushLoading();
				$newJquery.ajax($newJquery.extend({}, ajaxDefaults, source, {
					data: data,
					success: function(events) {
						events = events || [];
						var res = applyAll(success, this, arguments);
						if ($newJquery.isArray(res)) {
							events = res;
						}
						callback(events);
					},
	...
	function buildEventSource(sourceInput) { // will return undefined if invalid source
		...
		if ($newJquery.isFunction(sourceInput) || $newJquery.isArray(sourceInput)) {
			...
			source = $newJquery.extend({}, sourceInput); // shallow copy
		}
		...
		if (source) {
			...
			if ($newJquery.isArray(source.events)) {
				source.origArray = source.events; // for removeEventSource
				source.events = $newJquery.map(source.events, function(eventInput) {
	...
	function removeEventSource(source) {
		sources = $newJquery.grep(sources, function(src) {
			return !isSourcesEqual(src, source);
		});
		// remove all client events from that source
		cache = $newJquery.grep(cache, function(e) {
	...
	function getMiscEventProps(event) {
		...
		$newJquery.each(event, function(name, val) {
	...
	function removeEvents(filter) {
		...
		else if (!$newJquery.isFunction(filter)) { // an event ID
			eventID = filter + '';
		...
		cache = $newJquery.grep(cache, filter, true); // inverse=true

		// Remove events from array sources.
		// This works because they have been converted to official Event Objects up front.
		// (and as a result, event._id has been calculated).
		for (i=0; i<sources.length; i++) {
			if ($newJquery.isArray(sources[i].events)) {
				sources[i].events = $newJquery.grep(sources[i].events, filter, true);
			}
		}
		//Suresh commented Admin Calendar
		//reportEvents(cache);
	}
	
	
	function clientEvents(filter) {
		if ($newJquery.isFunction(filter)) {
			return $newJquery.grep(cache, filter);
		}
		else if (filter != null) { // not null, not undefined. an event ID
			filter += '';
			return $newJquery.grep(cache, function(e) {
				return e._id == filter;
			});
		}
	...
	function ensureVisibleEventRange(range) {
		...
		if (!range.end) {
			...
			range = $newJquery.extend({}, range); // make a copy, copying over other misc properties
		...
		if (abstractEvent) {
			if (abstractEvent._recurring) {
				...
				while (date.isBefore(_rangeEnd)) {
					// Admin Calendar change - Thamil
					if ((!dowHash || dowHash[date.day()]) && (date.isAfter(moment(abstractEvent.created)) || date.format('YYYY-MM-DD') == moment(abstractEvent.created).format('YYYY-MM-DD') )) { // if everyday, or this particular day-of-week
						...
						event = $newJquery.extend({}, abstractEvent); // make a copy of the original
						assignDatesToEvent(
							start, end,
							event.allDay, //!startTime && !endTime, // allDay? // Admin calendar change - Thamil (Retail allDay event property)
							event
						);
						events.push(event);
					}
	...
	function mutateEvent(event, newProps, largeUnit) {
		...
		$newJquery.each(newProps, function(name, val) {
			if (isMiscEventPropName(name)) {
				if (val !== undefined) {
					miscProps[name] = val;
				}
	...
	function mutateEvents(events, clearEnd, allDay, dateDelta, durationDelta, miscProps) {
		...
		$newJquery.each(events, function(i, event) {
			var oldProps;
			var newProps;
			...
			$newJquery.each(miscProps, function(name) {
			...
			$newJquery.extend(event, miscProps, newProps); // copy over misc props, then date-related props
			backupEventDates(event); // regenerate internal _start/_end/_allDay

			undoFunctions.push(function() {
				$newJquery.extend(event, oldProps);
				backupEventDates(event); // regenerate internal _start/_end/_allDay
			});
		});
	...
	function isExternalDropRangeAllowed(range, eventProps) {
		...
		if (eventProps) {
			eventInput = $newJquery.extend({}, eventProps, range);
	...
	function isRangeAllowed(range, constraint, overlap, event) {
		...
		range = $newJquery.extend({}, range); // copy all properties in case there are misc non-date properties
...
var BasicView = View.extend({
	...
	numberCellHtml: function(cell) {
		var date = cell.start;
		var classes;
		var cellGridClass = "fc-grid-" + cell.row + cell.col;
		if (!this.dayNumbersVisible) { // if there are week numbers but not day numbers
			return '<td/>'; //  will create an empty space above events :(
		}

		classes = this.dayGrid.getDayClasses(date);
		classes.unshift(cellGridClass);
...
var AgendaView = View.extend({
	...
	renderDates: function() {
		...
		this.bottomRuleEl = $newJquery('<hr class="fc-divider ' + this.widgetHeaderClass + '"/>')
...
fcViews.year = View.extend({
	...
	buildSkeleton: function(monthsPerRow, showNumbers) {
		...
		for (n=0; n<this.nbMonths; n++) {
			...
			var hiddenMonth = ($newJquery.inArray((m % 12), this.hiddenMonths) != -1);
		...
		this.table = $newJquery(s).appendTo(this.el);
		...
		this.table.find('.fc-year-monthly-name a').click(this.calendar, function(ev) {
			...
			ev.data.gotoDate([$newJquery(this).attr('data-year'), $newJquery(this).attr('data-month'), 1]);
		});

	
		this.dayBind(this.bodyCells);
	
		this.daySegmentContainer = $newJquery('<div style="position:absolute;z-index:8;top:0;left:0;"/>').appendTo(this.table);
	},
	...
	buildDayGrids: function() {
		...
		view.dayGrids = [];
		$newJquery.each(nums, function(offset, m) {
	...
	updateCells: function() {
		...
		this.subTables.each(function(i, _sub) {
			...
			$newJquery(_sub).find('.fc-bg').find('td.fc-day:first').addClass('fc-first');

			t.otherMonthDays[mi] = [0,0,0,0];
			$newJquery(_sub).find('.fc-content-skeleton tr').each(function(r, _tr) {
				if (r === 0 && t.dateInMonth(d,mi)) {
					// in current month, but hidden (weekends) at start
					t.otherMonthDays[mi][2] = d.date()-1;
				}
				$newJquery(_tr).find('td').not('.fc-week-number').each(function(ii, _cell) {
					var cell = $newJquery(_cell);
...
*/
	...
	tableByOffset: function(offset) {
		return $newJquery(this.subTables[offset]);
	},
	...
	setGridHeight: function(height, isAuto, grid) {
		...
		$newJquery.each(this.dayGrids, function(offset, dayGrid) {
			if (isAuto) {
...
(function($newJquery) {
	function monkeyPatchViewClass(View) {
		...
		View.prototype.registerRightclickListener = function() {
			...
			this.el.on('contextmenu', function(ev) {
				var eventElt = $newJquery(ev.target).closest('.fc-event');
				if (eventElt.length) {
					...
					var fcContainer = $newJquery(ev.target).closest(
	...
	var fc = $newJquery.fullCalendar;
	monkeyPatchViewClass(fc.views.agenda);
	monkeyPatchViewClass(fc.views.basic);
})($newJquery);
function validatenexthreshold(range){
	return (range.event.allDay && range.event.dayDifference>=1)?true:false;
}