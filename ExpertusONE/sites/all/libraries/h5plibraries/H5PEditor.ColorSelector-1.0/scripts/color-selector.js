...
H5PEditor.widgets.colorSelector = H5PEditor.ColorSelector = (function ($) {
  ...
  ColorSelector.prototype.appendTo = function ($wrapper) {
    ...
    $('<span>', {
      ...
      html: window.parent.Drupal.t(self.field.label)
    ...
    $('<span>', {
        ...
        'onClick':'closeBGSelector()',
        html: "<input type='button' value='"+H5PEditor.t('H5PEditor.CoursePresentation', 'close')+"' class='bg-selector-close' id='bg-selector-close'></input>"
      }).appendTo(self.$container); 
    var config = {
      preferredFormat: 'hex',
      color: self.getColor(),
...
{
	H5P.jQuery(".h5p-slidecontrols-button-background").click();
}