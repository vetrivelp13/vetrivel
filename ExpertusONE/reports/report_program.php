...
class report_program extends ReportSyncUp {
	...
	protected function createTempTable() {
		try {
			...
			$createTable = "CREATE TABLE " . $this->temp_table_name ." (
              ...
              `TrainingPlanObjectCode` varchar(50) NOT NULL,
	...
	protected function tempTableDataPopulate() {
		try {
			$tempDataQuery = "insert into ".$this->temp_table_name."
					SELECT
						concat_ws('-', `prm`.`id`, `sg`.`id`, `mcm`.`id`) AS `RUID`,
						...
						`prm`.`object_type` 		AS `TrainingPlanObjectCode`,
					...
					INNER JOIN `slt_profile_list_items` `stat` ON `prm`.`status` = `stat`.`code`
					INNER JOIN `slt_profile_list_items` `pli1` ON `pli1`.`code` = `prm`.`object_type`
					LEFT JOIN `slt_profile_list_items` `lang` ON `lang`.`code` = `prm`.`lang_code`
					where ((prm.updated_on > '".$this->updSync_on ."') OR (sg.updated_on > '".$this->updSync_on ."') OR (mcm.updated_on > '".$this->updSync_on ."'))
	...
	protected function reportUpdate() {
		...
		$query = "update ". $this->report_table ." flat
						...
						set
							...
							`flat`.`TrainingPlanObjectCode` = `temp`.`TrainingPlanObjectCode`,
	...
	protected function reportCollectIds($action) {
		try {
			$query = "SELECT 
					...
					LEFT JOIN `slt_module` `sg` ON `prm`.`id` = `sg`.`program_id`
					LEFT JOIN `slt_module_crs_mapping` `mcm` ON `mcm`.`module_id` = `sg`.`id`
						where prm.updated_on > '".$this->delSync_on ."'";
			return $query;
		} catch(Exception $ex){
