...
//		$('#'+this.getUniqueWidgetId()+"profilemain").append('<input type="button" value="test" onclick="'+ this.getWidgetInstanceName() +'.test();" />');
		...
		/*$("#R").find("a").each(function(){
			//alert("t");
			$(this).after('<strong><a href="#" onclick="'+ obj.getWidgetInstanceName() +'.showDialog();"> + </a>&nbsp;<a href="#"> / </a>&nbsp;<a href="#"> &minus; </a></strong>');
		});*/
		}catch(e){
			// to do
		}
	...
	this.showDialog = function(){
		try{
			//alert(this.renderDialog);
			//this.renderDialog({title:'Add Root Profile',resizable:true,draggable:true,autoOpen:true,isCloseOnEscape:true,width:300,position:[centerWidth,centerHeight],dialogid:'addprofile',height:'auto',destroyable:true,body:'<div id=\'newprofile\'></div>',modal:true});
			//this.renderDialog({title:'Add Catalog',resizable:false,draggable:false,autoOpen:true,isCloseOnEscape:false,width:600,position:[centerWidth,centerHeight],dialogid:'catlogcontent',height:'auto',destroyable:true,body:'<div id=\'newcatalog\'></div>',modal:true});
			//this.renderDialog({title:'Add Root Profile',resizable:false,draggable:false,autoOpen:true,isCloseOnEscape:true,width:500,position:[500,200],dialogid:'addprofile',height:'auto',destroyable:true,body:'<div id=\'newprofile\'></div>',modal:true});
			var t = $.tree.focused();
	...
	this.renderResults=function(){
		...
		if(this.mode=="getprofile"){
			...
			if(this.level==1){
				...
				dialogBody += '<tr><td>'+SMARTPORTAL.t("Description :")+'</td><td><input type="text" id="prof_desc" value="" /></td>';
				dialogBody += '<td>'+SMARTPORTAL.t("Active :")+'</td><td><select id="prof_active"><option value="Y">'+SMARTPORTAL.t("Yes")+'</option><option value="N">'+SMARTPORTAL.t("No")+'</option></select></td></tr>';
				dialogBody += '<tr><td colspan="2" align="center"><input type="button" value="Save" onclick="'+this.getWidgetInstanceName()+'.updateRootNode();" /></td><td colspan="2"><input type="button" value="Close" onclick="$('+'\"#newprofile\"'+').remove();"/></td></tr></table></div>';
