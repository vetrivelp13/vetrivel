...
class ZipFolder {
					...
					while ($zip_entry = zip_read($zip)) 
					{
						if (zip_entry_open($zip,$zip_entry,"r")) 
						{
							...
							if ($directory_name != ".") 
							{
								...
								{
									$dir_op = $dir_op . $k;
									expDebug::dPrint("Inside While loop IF2 foreach1 dir_op");
									expDebug::dPrint($dir_op);
	...
	public function openZip(){
		...
		$this->makeDirectories($this->getSavePath().$this->getFolder());
		if ($zip->open($this->getLoadPath().$this->getFile()) === TRUE) {
		    $zip->extractTo($this->getSavePath().$this->getFolder());
		    $zip->close();
		    return true;
		} else {
		    return false;
