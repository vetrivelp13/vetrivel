...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			// console.log('ts.p.jsonReader = ', ts.p.jsonReader);
			...
			// console.trace();
			// console.log('ts.p.keyIndex', ts.p.keyIndex, 'dReader.id', dReader.id, 'idn = ', idn);
			if(f.length>0 && !isNaN(idn)) {
				if (ts.p.remapColumns && ts.p.remapColumns.length) {
					idn = $.inArray(idn, ts.p.remapColumns);
				}
			...
			// console.log('dReader.root', dReader.root, 'data', data);
			if (!drows) { drows = []; }
			...
			while (i<len) {
				...
				cdr = $.jgrid.getAccessor(cur,'class');
				cdr = (cdr) ? " "+cdr : ""; 
				// console.log('idr', idr,'cdr=', cdr);
				// console.log( 'cur=', cur);
				if(idr === undefined) {
					idr = br+i;
					if(f.length===0){
				...
				rowData.push("<tr"+hiderow+" id=\""+ idr +"\" tabindex=\"-1\" role=\"row\" class= \"ui-widget-content ui-row-"+idr +" jqgrow ui-row-"+ts.p.direction+""+cn1+cdr+"\" data-rowid=\""+ idr +"\" data-jqrid=\""+ $.jgrid.randId() +"\">");
				if(ts.p.rownumbers===true) {
        ...
        setPager = function (pgid, tp){
			...
			$("#"+$.jgrid.jqID(pgid) )
			.append("<div id='"+pgcnt+"' class='ui-pager-control' role='group'><div id='selBeforeTable'></div><div id='selTable'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' role='row'><tbody><tr><td id='"+lft+"' align='left'></td><td id='"+cent+"' align='center' style='white-space:pre;'></td><td id='"+rgt+"' align='right'></td></tr></tbody></table></div><div id='selAfterTable'></div></div>")
...
//Modified by VJ Datatable page view
			if(ts.p.rowList.length >0){
				str = "<td dir='"+dir+"'>";
				str += "<div id='data-table-page-view'><span>"+Drupal.t('LBL816')+"</span>";
				if(currTheme == "expertusoneV2"){
					str +="<span class=\"page-total-view\" id='"+ts.p.rowList.length+"'></span>";
					str +="<span class=\"page_count_view\" id=\"0\">"+ts.p.rowList[0]+"</span>";
					str +="<span class=\"page_next_prev-container\" id=\"\">";
					if((ts.p.rowNum == ts.p.rowList[0])) {
						str  +="<span class=\"page-show-next\"></span>";
						str +="<span class=\"page-show-prev hide-pointer\"></span>";
					}
					else if((ts.p.rowNum == ts.p.rowList[ts.p.rowList.length-1])) {
						str  +="<span class=\"page-show-next hide-pointer\"></span>";
						str +="<span class=\"page-show-prev \"></span>";
					}
					else {
						str  +="<span class=\"page-show-next\"></span>";
						str +="<span class=\"page-show-prev\"></span>";
				...
				str +="<span>"+Drupal.t('MSG588')+"</span></div></td>";
			}
			if(dir=="rtl") { pgl += str; }
			var pageText = (currTheme == "expertusoneV2") ?"<span>"+Drupal.t('LBL989')+"</span>" : ""; 
			//if(ts.p.pginput===true) { pginp= "<td class='search-pagination-right' dir='"+dir+"'>"+$.jgrid.format(ts.p.pgtext || "",pageText+"<input class='ui-pg-input' type='text' size='2' maxlength='7' value='0' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>")+"</td>";}
			if(ts.p.pgbuttons===true) {
				var po=["first"+tp,"prev"+tp, "next"+tp,"last"+tp]; if(dir=="rtl") { po.reverse(); }
				if(currTheme == "expertusoneV2" && pginp !== ""){
					pgl += pginp !== "" ? sep+pginp+sep:"";
					pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon ui-icon-seek-pgstart'></span></td>";
				...
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon ui-icon-seek-pgend'></span></td>";
			...
			if(currTheme == "expertusoneV2"){
				$('.page-show-prev',"#"+pgcnt).bind('click',function() {
					if(tp) {var pageCntSpan = parseInt($(".page_count_view",ts.p.pager).attr('id'))-1;$(".page_count_view",ts.p.pager).attr('id',pageCntSpan);}
					else if(ts.p.toppager) {var pageCntSpan = parseInt($(".page_count_view",ts.p.toppager).attr('id'))-1;$(".page_count_view",ts.p.toppager).attr('id',pageCntSpan);}
					if(ts.p.rowList[pageCntSpan]){
							ts.p.page = Math.round(ts.p.rowNum*(ts.p.page-1)/ts.p.rowList[pageCntSpan]-0.5)+1;
							ts.p.rowNum = ts.p.rowList[pageCntSpan];
							if(tp) { $('.page_count_view',ts.p.pager).html(ts.p.rowList[pageCntSpan]); $('.page_count_view',ts.p.pager).attr('id',pageCntSpan);}
							else if(ts.p.toppager) { $('.page_count_view',ts.p.toppager).html(ts.p.rowList[pageCntSpan]); $('.page_count_view',ts.p.toppager).attr('id',pageCntSpan);}
							if(!clearVals('records')) { return false; }
							populate();
							if((ts.p.rowNum == ts.p.rowList[0])) {
								$("span.page-show-prev").addClass('hide-pointer');
								$("span.page-show-next").removeClass('hide-pointer');
							}
							else if((ts.p.rowNum == ts.p.rowList[ts.p.rowList.length-1])) {
								$("span.page-show-prev").removeClass('hide-pointer');
								$("span.page-show-next").addClass('hide-pointer');
							}
							else {
								$("span.page-show-prev").removeClass('hide-pointer');
								$("span.page-show-next").removeClass('hide-pointer');
				...
				$('.page-show-next',"#"+pgcnt).bind('click',function() {
					if(tp) {var pageCntSpan = parseInt($(".page_count_view",ts.p.pager).attr('id'))+1;$(".page_count_view",ts.p.pager).attr('id',pageCntSpan);}
					else if(ts.p.toppager){var pageCntSpan = parseInt($(".page_count_view",ts.p.toppager).attr('id'))+1;$(".page_count_view",ts.p.toppager).attr('id',pageCntSpan);}
					if(ts.p.rowList[pageCntSpan]){	
							ts.p.page = Math.round(ts.p.rowNum*(ts.p.page-1)/ts.p.rowList[pageCntSpan]-0.5)+1;
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			//Changes done by Vincent on Aug 12, 2013 for hide empty header in grid
			if(ts.p.colNames[i]==''){
				thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+dir+" data-grid-header-merge'"+ tooltip+">";
			}else{
		...
		$("th",thr).each(function ( j ) {
			...
			if( !(nm == 'cb' || nm=='subgrid' || nm=='rn') ) {
				if(ts.p.viewsortcols[2]){
					if(sort)
                        $("div",this).addClass('ui-jqgrid-sortable').css("cursor","pointer");
                    else
                        $("div",this).addClass('ui-jqgrid-sortable un-sortable').css("cursor","default");
				}
			}
			if(sort) {
		...
		$(ts).bind('mouseover',function(e) {
			...
			if ($.isEmptyObject(ptr[0])) 
				ptr = $(e.target).closest("tr.jqgcrow");
			prrRowId = $(ptr).data('rowid');
			if($(ptr).attr("class") !== "subgrid") {
				$(ptr).addClass("ui-state-hover");
				$('tr.ui-row-' + prrRowId).addClass("ui-state-hover");
			}
			return false;
			...
			$('tr.ui-row-' + prrRowId).removeClass("ui-state-hover");
			return false;
		});
		}
		var ri,ci;
		$(ts).before(grid.hDiv).click(function(e) {
			//console.log('hDiv click');
			td = e.target;
			...
			cSel = true;
			//update last accessed row
			// console.log('ts ', ts, 'ptr ', ptr[0].id, 'e ', e);
			ts.p.lastAccessedRow = $(e.target).closest("tr.jqgrow")[0];
...
$.jgrid.extend({
	...
	setRowData : function(rowid, data, cssp) {
		...
		// console.log('rowid', rowid, 'data', data, 'cssp', cssp);
		this.each(function(){
				...
				try {
					$(this.p.colModel).each(function(i){
						...
						if( data[nm] !== undefined) {
							...
							nm = this.name;
							lcdata[nm] = this.formatter && typeof(this.formatter) === 'string' && this.formatter == 'date' ? $.unformat.date(data[nm],this) : data[nm];
							vl = t.formatter( rowid, data, i, data, 'edit');
							$("td:eq("+i+")",ind).html(vl);
							// console.log('vl in else', vl);
							// console.log('vl in else');
						}
					});
					if(t.p.datatype == 'local') {
						var pos = t.p._index[rowid];
		...
		if(rdata) {
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					row = "<tr id=\""+rowid+"\" role=\"row\" tabindex=\"-1\" class=\"ui-widget-content jqgrow ui-row-"+t.p.direction+" "+cna+"\"  data-jqrid=\""+ $.jgrid.randId() +"\">" + row+"</tr>";
					if(t.rows.length === 0){
	...
	getjqGridObject: function() {
		return this[0].grid;
		
	},
	// if pageNo number is not given next page data will be appended to the given grid
	appendDataToGrid: function(gridOptions) {
		try {
			//console.trace();
			var _this = this;
		    var grid = _this[0].p;
		    var ts = _this[0];
		    var lastPostData = grid.postData;
		    var _grid = _this[0].grid;
		    var lastPostData = grid.postData;
			//console.log('postData=', grid.postData);
		    ts.p.postData = {
							  page: (gridOptions.pageNo !== undefined ? gridOptions.pageNo : (lastPostData.page + 1)),
							  rows: lastPostData.rows,
							  totalrows: parseInt(grid.records)
						  }
	 		$.ajax($.extend({
			url: ts.p.url,
			type: ts.p.mtype,
			...
			success: function(data, st) {
					try {
						typeof gridOptions.onSuccess != 'undefined' && gridOptions.onSuccess();
					    ...
					    if (rows !== undefined && rows.length > 0) {
					    	typeof gridOptions.beforeAppend != 'undefined' && gridOptions.beforeAppend();
					    	//if there are new JSONData added to the existing list of enrollments
					    	$.each(rows, function (index, row) {
					    		var rowCellID = (row['jqgrid-rowid'] === undefined ? row['cell']['jqgrid-rowid'] : row["jqgrid-rowid"]);
					    		if (rowCellID === undefined || rowCellID == null) {
					    			rowCellID = (row['id'] === undefined ? row['cell']['id'] : row["id"]);
					    		}
					    		_this.addRowData(rowCellID, row['cell'], "last");
					    		// this.addRowData(row["id"], row['cell'], "last");
					    	});
					    	typeof grid.loadComplete != 'undefined' && grid.loadComplete(data);
					    	typeof gridOptions.afterAppend != 'undefined' && gridOptions.afterAppend(data, rows.length);
					    	//typeof gridOptions.afterAppend != 'undefined' && gridOptions.afterAppend();
					    } else {
					    	typeof gridOptions.noData != 'undefined' && gridOptions.noData();
					    }
					} catch (e) {
			...
			error: function(xhr,st,err) {
				if($.isFunction(ts.p.loadError)) { ts.p.loadError.call(ts,xhr,st,err); }
				typeof gridOptions.onError != 'undefined' && gridOptions.onError();

			},
			beforeSend: function(xhr) {
				if($.isFunction(ts.p.loadBeforeSend)) { ts.p.loadBeforeSend.call(ts,xhr); }
				typeof gridOptions.beforeSend != 'undefined' && gridOptions.beforeSend();
			},
			complete: function() {
				typeof gridOptions.onComplete != 'undefined' && gridOptions.onComplete();
			}
		},$.jgrid.ajaxOptions, ts.p.ajaxGridOptions));
	...
	updateRowByRowId: function(gridOptions) {
		try {
			var rowId = gridOptions.rowId;
			var _this = this;
		    var grid = _this[0].p;
		    var ts = _this[0];
		    var _grid = _this[0].grid;
			// console.log('ts.p', ts.p);
			...
			dataType: ts.p.dataType,
			data: data,
			// data: {'ent_id': 23, 'ent_type': 'cre_sys_obt_trp', 'rows': 1},
			success: function(data, st) {
					try {
						// typeof gridOptions.onSuccess != 'undefined' && gridOptions.onSuccess();
					    ...
					    if(rows !== undefined && rows.length > 0) {
							...
							$(ts.p.colModel).each(function(i){
								data[this.name] = row['cell'];
							});
							var updateRow = _this.setRowData(rowCellID, data, "last-updated"); */
							row['cell']['updateSingleRow'] = true;
							var updateRow = _this.setRowData(rowCellID, row['cell'], "last-updated");
							 // console.log(updateRow);
							 // console.log(_this.setRowData(rowCellID, {cell: row['cell']}, "last-updated"));
						// this.addRowData(row["id"], row['cell'], "last");
							});
							typeof grid.loadComplete != 'undefined' && grid.loadComplete(data, null, null, false);
							// typeof gridOptions.afterAppend != 'undefined' && gridOptions.afterAppend();
					//typeof gridOptions.afterAppend != 'undefined' && gridOptions.afterAppend();
					    } else {
							// typeof gridOptions.noData != 'undefined' && gridOptions.noData();
