...
$.widget("ui.learningplan", {
  ...
  updateLPMultiContentLaunchDialog : function(enrollmentId, lessonId, lessonInfo, contentQuizStatusConsolidated) {
    ...
    if(lessonInfo.LesScore !=null && lessonInfo.LesScore!=undefined && lessonInfo.LesScore != '' && lessonInfo.LesScore != '0.00') {
    	lesnscore = pipe1 + Drupal.t('LBL668') + ' : ' + lessonInfo.LesScore;
    }
    ...
    if (lessonInfo.ContentStatus != '') {
    	contentStatus = Drupal.t(lessonInfo.ContentStatus);
    }
    if(lessonInfo.LaunchType == 'VOD' && lessonInfo.Status != ''){
    	contentStatus = Drupal.t(lessonInfo.Status);
    }
    var score = '';
    ...
    if(lessonInfo.ContScore !=null && lessonInfo.ContScore!=undefined && lessonInfo.ContScore != '' && lessonInfo.ContScore != '0.00' ) {
      score = pipe + Drupal.t('LBL668') + ' : ' + lessonInfo.ContScore;
    }
    var attemptsLeft = (lessonInfo.AttemptLeft == 'notset')? '' : lessonInfo.AttemptLeft;
    if (attemptsLeft !== '') {
    	attemptsLeft = pipe + Drupal.t('LBL2027')+' : ' + attemptsLeft;
    }
    
    var attemptsLeft = (lessonInfo.AttemptLeft == 'notset')? '' : lessonInfo.AttemptLeft;
    if (attemptsLeft !== '') {
    	attemptsLeft = pipe + Drupal.t('LBL2024')+' : ' + attemptsLeft;
    }
...
//    	var remValidityDays = validityDay - diffDays;
		...
		validityDay = pipe + Drupal.t('LBL604') +' : ' + remValidityDays + ' ' + daysLBL;
    }
	...
	if(lessonInfo.LaunchType == 'VOD') {
		...
		percentCompleted = pipe1 + progress+'% '+Drupal.t('Completed');
	}
	...
	callbackLPLoader : function(data, postdata, formid, updateShowMore){
	 ...
	 // console.log(data, postdata, formid, updateShowMore);
	  ...
	  if(document.getElementById('learningplan-tab-inner')){
			var objData = $('#paintEnrollmentLPResults').data("lastPostData") === undefined ? {} : $('#paintEnrollmentLPResults').data("lastPostData");
			//console.log('objData in cp js');
			//console.log(objData);
			var clickStyle = $('#cp-modal').css("display");
			if((objData.objecttype != 'cre_sys_obt_cls' || objData.updateFrom=='LP') && objData.enrId!==undefined && (clickStyle == "none" ||  clickStyle == undefined || clickStyle == 'undefined')){
				$('#prg_'+objData.masterEnrId+'SubGrid').remove();
				$('#lp_seeless_prg_'+objData.masterEnrId).click();
				$('#paindLPContentResults_prg_'+objData.masterEnrId).show();
				if(document.getElementById('lp_seemore_prg_'+objData.enrId))
					$('#lp_seemore_prg_'+objData.enrId).click();
				else{
					if(objData.masterEnrId!==undefined){
							$('#lp_seemore_prg_'+objData.masterEnrId).click();
							var enrID = objData.enrId;
							setTimeout(function(){$('#lp_class_seemore_'+enrID).click();},2000);
					}
				}
			}
	 }
	...
	paintLearningPlanTitle : function(cellvalue, options, rowObject) {
		...
		var completedPercentage 	= enr_comp_status['overall_progress'];
...
//		}
	    ...
	    if(overall_status == 'lrn_tpm_ovr_cmp' || overall_status == 'lrn_tpm_ovr_enr' ||  overall_status == 'lrn_tpm_ovr_inp' ) {
	    	var obj = options.colModel.widgetObj;
	    	var dataId = 'prg_' + master_enroll_id;
	    	var launch_data = {
					masterEnrollId : master_enroll_id,
					enrollId :	0,
					programId : prg_id, 
					courseId : 0, 
					classId  : 0,
					userId : user_id,
					LaunchFrom : "LP",	
				//	classTitle:htmlEntities(prg_title),
					pagefrom : "lp_seemore",
					containerId : dataId
			};
			
			launch_data = objectToString(launch_data);
			html += '<div class="lp_seemore" id="lp_seemore_'+dataId+'" onclick="$(\'#learningplan-tab-inner\').data(\'contentPlayer\').playContentMylearning(' + launch_data + ');">'+ Drupal.t('LBL713') +'</div>';
			html += '<div class="paindContentResults lsSeeMorePlaceholderdiv" id="paindLPContentResults_'+dataId+'"></div>';
	...
	paintLPActions : function(cellval,options,rowObject){
		...
		var dataId	= 'prg_' + master_enroll_id;
		var completedPercentage 	= rowObject['enr_comp_status']['overall_progress'];
		...
		if ((assessStatus == 1 && maxScoreValidationpost == 1) || (preassessStatus == 1 && maxScoreValidationpre == 1)) 
			isLaunchable = true;
		...
		if(overall_status == 'lrn_tpm_ovr_enr' || overall_status == 'lrn_tpm_ovr_inp' || overall_status == 'lrn_tpm_ovr_cmp') {
		  	var dataId = 'prg_' + master_enroll_id;
			if (mylearn_content_launchable == true || isLaunchable) {
				var launch_data = {
						masterEnrollId : master_enroll_id,
						enrollId :	0,
						programId : prg_id, 
				...
				launch_data = objectToString(launch_data);
				// launch action - training plans
				html += '<a id="prg-launch-prg_'+master_enroll_id+'" href="javascript:void(0);" data="'+data1+'" class="actionLink enroll-launch" onclick="$(\'#learningplan-tab-inner\').data(\'contentPlayer\').playContent(' + launch_data + ');">'+Drupal.t("LBL199")+'</a>';
			} else {
				sharebutton = true
				// default action
		...
		if(prg_object_type == 'cre_sys_obt_crt' && recertify_path > 1 && (hideShare == 0 || mylearn_content_launchable == true || isLaunchable)){
			//	var recertifyObj = '$("body").data("learningcore")';
			//	var recertifyFrom = 'N';
			//	var loaderDiv = 'enroll-lp-result-container';
               //if(hideShare == 0){
			...
			if(prg_status == 'lrn_lpn_sts_atv' && sharebutton == null){
                            if(hideShare == 0){
			...
			if (overall_status == 'lrn_tpm_ovr_enr'){
			enrollAction += "<li class='action-enable'><a href='javascript:void(0);' class='actionLink' data=\"" + data1 +
			...
			// console.log(dataId, completedPercentage, 'lp_progress');
			setTimeout(function () {
			progressBarRound(dataId, completedPercentage, 'lp_progress','progress_',this);
			//$("#learningplan-tab-inner").data("learningplan").progressbarlp(dataId, completedPercentage, 'lp_progress');
			}, 200);
		...
		if (overall_status == 'lrn_tpm_ovr_enr' || overall_status == 'lrn_tpm_ovr_inp' || overall_status == 'lrn_tpm_ovr_cmp') {
			...
			//	if(content_pending_flag==true){
				//	html+='<div class="clsSurveyPendingLable">'+Drupal.t("Completed")+' '+ Drupal.t("Content")+' '+ Drupal.t("Pending")+'</div>';
				//}
				//else
				 if(mylearn_survey_pending_flage == true){
					//html+='<div class="prgSurveyPendingLable">'+Drupal.t("Survey")+' '+ Drupal.t("Pending")+'</div>';
           html+='<div class="prgSurveyPendingLable"><a id = "take_survey_lp" onclick="$(\'#learner-enrollment-tab-inner\').data(\'contentPlayer\').launchsurvey('+launch_data+');" >'+Drupal.t("LBL3212")+' '+ Drupal.t("Survey")+'</a></div>';
	...
	addAccordionToTableLPView : function(prg_id,openCloseClass,openpos,closepos,childClass,callback,obj,parentObj,isRemove) {
		...
		resetFadeOutForAttributes('#course-detail-section-'+data.MasterEnrollId,'main-item-container','line-item-container','container','myprogramtp');
		}catch(e){
			// to do
		}
	},
	...
	paintModuleDetails : function(result) {
		try{
			...
			for(c=0; c < result.length ; c++){
				...
				var completedPercentage 			= result[c]['enr_comp_status']['overall_progress'];
				var mylearn_content_launchable   	= result[c]['mylearn_content_launchable'];
				var mylearn_survey_pending_flage 	= result[c]['mylearn_survey_pending_flage'];
				var total_content_count 			= result[c]['total_content_count'];
				var content_pending_flag 			= result[c]['content_pending_flag'];
				var lnrAttach						= result[c]['show_lnr_lp_attach'];
				var isLaunchable					= false;
				var expires_on_lbl                  = result[c]['expires_on_lbl'];
				console.log(expires_on_lbl);
				if(recertify_path > 1) {
			...
			postassObj =surObj+".callTakeSurveyToClass(" + prg_id + ",\"" +
				...
				//var param="data={'MasterEnrollId':1','PrgId':'130'}";
				...
				var enrollAction = ''; 			
			 /* // Share action disabled due to cp
			enrollAction += '<li class="action-enable"><a href="javascript:void(0)" class="actionLink" data=\"' 
				+ data1 +'\" name="Refer" onclick="$(\'body\').data(\'refercourse\').getReferDetails(\''+ prg_id +'\',\'' +prg_object_type+
				'\',\'enroll-lp-result-container\',\''+prg_type+'\');" >' + Drupal.t("Share") + '</a></li>';*/
			
			 enrollAction += "<li class='action-enable'><a href='javascript:void(0)' class='actionLink' data=\"" +
			  data1 + "\" name='Certificate' onclick='window.open(\"printcertificate.php?enrollid=" + master_enroll_id_encrypted +
			  "&certifyfrom=LP&classid=" + prg_id_encrypted + "&userid=" + userId +
			  "\", \"PrintCertificate\"" +
			  ", \"width=1050, height=900, toolbar=yes, location=yes, directories=yes, status=yes, menubar=yes, " +
			  "scrollbars=yes, copyhistory=yes, resizable=yes\")'>" + Drupal.t("LBL205") + "</a></li>";
				
	 			//html += '<tr class="set-height ui-widget-content" id ="lp-crs-'+prg_id+'-course-'+recertify_path+'">';
				//rhtml += '<tr class="set-height ui-widget-content" id ="prg_3"'+master_enroll_id+'>';
				...
				rhtml += '<tr class="jqgcrow ui-row-ltr ui-widget-content ui-row-'+moduleId+'" id ="'+moduleId+'" data-rowid="'+moduleId+'">';				
	 			...
	 			rhtml += '<div class="enroll-lp-course-title"><span id="lp_attachment_'+master_enroll_id+'" data="'+attachdata+'"></span><span id="titlePrgEn_'+master_enroll_id+'" class="item-title limit-title-row ' + overall_status + '" ><span class="limit-title limit-title-lp enroll-lp-class-title vtip" title="'+prg_title+'" href="javascript:void(0);">'+prg_title+'</span></span></div>';
	 			...
	 			if(overall_status == 'lrn_tpm_ovr_cmp')
	 				...
	 				if(prg_shortdesc != null && prg_shortdesc != 'null') {
	 			    	...
	 			    	rhtml += '<div class="recordDiv FindTrngTxt"><span class="limit-desc limit-desc-lp-mod vtip" id="LPShortDesc_mod_'+master_enroll_id+'"><span class="cls-learner-descriptions">'+prg_description_icon+'</span></span></div>';
	 			    	rhtml += '</div>';
	 				}
	 		    ...
	 		    if(overall_status == 'lrn_tpm_ovr_cmp') {
	 				...
	 				launch_data = objectToString(launch_data);
	 				rhtml += '<div class="lp_seemore" id="lp_seemore_'+dataId+'" onclick="$(\'#learningplan-tab-inner\').data(\'contentPlayer\').playContentMylearning(' + launch_data + ');">'+ Drupal.t('LBL713') +'</div>';
	 				rhtml += '<div class="paindContentResults lsSeeMorePlaceholderdiv" id="paindLPContentResults_'+dataId+'"></div>';
 				...
 				if((overall_status == 'lrn_tpm_ovr_cmp')&&(isLaunchable)) {
 					var dataId = 'prg_' + master_enroll_id;
					var launch_data = {
							masterEnrollId : master_enroll_id,
							enrollId :	0,
					...
					launch_data = objectToString(launch_data);
					// launch action - training plans
					rhtml +='<li class="action-enable" style="display:none"><a id="prg-accodion-mod_'+master_enroll_id+'" href="javascript:void(0);" data="'+data1+'" class="actionLink" onclick=\''+objEval+'.addAccordionToTableLP1('+prg_id+',this.className,"0 0","0 -61px","dt-child-row-En",'+objEval+'.getPrgDetails,this,'+objEval+',true); \'></a></li>' +
		 		    	'<a id="prg-launch-prg_'+master_enroll_id+'" href="javascript:void(0);" data="'+data1+'" class="actionLink enroll-launch-full" onclick="$(\'#learningplan-tab-inner\').data(\'contentPlayer\').playContent(' + launch_data + ');">'+Drupal.t("LBL199")+'</a>';
				} else {
	 		    ...
	 		    if (overall_status == 'lrn_tpm_ovr_enr' || overall_status == 'lrn_tpm_ovr_inp' || overall_status == 'lrn_tpm_ovr_cmp') {
	 		    	rhtml += '<div class="progress" id="progress_'+dataId+'"></div>';	
	 				if(overall_status == 'lrn_tpm_ovr_cmp') {
	 					if(content_pending_flag==true){
	 						rhtml+='<div class="clsSurveyPendingLable">'+Drupal.t("Completed")+' '+ Drupal.t("Content")+' '+ Drupal.t("Pending")+'</div>';
	 					}
	 					else if(mylearn_survey_pending_flage == true && content_pending_flag==false){
	 						rhtml+='<div class="prgSurveyPendingLable">'+Drupal.t("Survey")+' '+ Drupal.t("Pending")+'</div>';
	 					}
	 				}
					// console.log('paintModuleDetails', dataId, completedPercentage, 'lp_mod_progress');
					progressBarRound(dataId, completedPercentage, 'lp_mod_progress','progress_',this);
	 				//$("#learningplan-tab-inner").data("learningplan").progressbarlp(dataId, completedPercentage, 'lp_mod_progress');
	 			}	 
	...
	getLPClassDetail : function(data){
		...
		if($('#course-detail-section-'+masterEnrollId).html() == ''){
			...
			if ($('#paindLPContentResults_mod_'+masterEnrollId).length >= 1)
				$('#paindLPContentResults_mod_'+masterEnrollId).css('display', 'block');
	...
	paintLPClassDetails : function(result,data){
		...
		if(res_length > 0 ) {
			...
			if(data.mro_type !=null && data.mro_type !='null' && data.mro_type !='' && data.mro_type !='Mandatory' && data.mro_type !='Recommended' && data.mro_type !='Optional') {
				...
				var MROValue = (data.mro_type!=null  ? (data.mro_type) : '');
					...
					if(data.overall_status =='lrn_tpm_ovr_cln'){
						...
						html += Drupal.t('LBL680')+" : " + "<span class='enr-status-cls'>" + ((userId != data.UpdatedBy) ? updatedByName : Drupal.t('LBL3046')) + '</span>';
						html += "</span>";
					}
					if(data.overall_status =='lrn_tpm_ovr_cmp'){
						...
						html += "<span title='"+updated_by_ins_mngr_slf+"'>";
						html += Drupal.t('LBL681')+" : " +  "<span class='enr-status-cls'>" +  ((userId != data.UpdatedBy) ? updatedByName : Drupal.t('LBL3046')) + '</span>';
						html += "</span>";
					}
				html += "</div>";
			}
			...
			html += '<table class="enroll-show-moreclass" id="selMaseterEnrollId-'+masterEnrollId+'" width="100%" border="0" cellpadding="0" cellspacing="0">';
			html += '<tr>';
			html += '<th colspan="3" class="'+thClass+'"></th>';
			html += '</tr>';
			// console.log(Object.keys(group_name_arr).length);
			  ...
			  //console.log('grp name ' + result[c]['groupname'])			 
			 		if(key == result[c]['groupname'] || (result[c]['groupname'] == 'null' || result[c]['groupname'] == null || result[c]['groupname'] == '')){
						...
						if(this.currTheme == "expertusoneV2"){
							...
							html += '<td class="lp-class-delivery-type"><div class="delivery-type-icon ' + dataDelTypeImage + ' vtip" title="' + Drupal.t(dataDelType) + '" style="border-right: 0px none;"></div></td>';	// dellivery type icon
...
//									var remValidityDays = result[c].launch[0].ValidityDays - diffDays;
								...
								//h5pcustomize, do not display version
								//if(result[c].launch[0].LearnerLaunchURL !=null && result[c].launch[0].LearnerLaunchURL.indexOf("h5p/embed")>0)
								if(result[c].launch[0].subtype != undefined && (result[c].launch[0].subtype == "h5p-presentatn" || result[c].launch[0].subtype == "h5p-Youtube" ||  result[c].launch[0].subtype == "h5p-Vimeo"  || result[c].launch[0].subtype == "h5p-video-mp4" || result[c].launch[0].subtype == "h5p-video-webm"))
									VersionNo = "";
...
//						console.log('percentCompleted = ', percentCompleted);
						/*if(baseType == 'VOD') {
							enrollClassInfo += (enrollClassInfo!= '' ? pipe : '') + percentCompleted;
						}*/
//						if(Drupal.settings.mylearning_right===false)
...
//							html += ((classScore != '') ? (classScore) : '');
							...
							enrollClassInfoDet = ((courseQuizStatusString != '' && ( (ValidityDays != '' && validity_flag==false) || classScore != ''  || classScore=='')) ? (pipe) : '');/*Viswanathan added for #79194*/
							enrollClassInfo += "<div class = 'line-item-container float-left'>"+enrollClassInfoDet+"</div>";
						...
						enrollClassInfo += "<span id='compLpDetails"+dataId+"' class='compLpDetails'>";
...
//							html += ((ValidityDays != '' && validity_flag==false) ? (ValidityDays) : '');
		...
		//html += '</div>';	
					  ...
					  if(parseInt(Object.keys(group_name_arr).length) !== parseInt(g) && group_name_count[key]==0 && group_name_flag===true)
			 		  ...
			 		  if((RegStatusCode != 'lrn_crs_cmp_inc' && RegStatusCode !='lrn_crs_reg_can' && RegStatusCode !='lrn_crs_cmp_nsw' && RegStatusCode != 'lrn_crs_reg_ppm' && RegStatusCode != 'lrn_crs_reg_wtl'
			 			  	&& (overallStatus == 'lrn_tpm_ovr_enr' || overallStatus == 'lrn_tpm_ovr_inp' || overallStatus == 'lrn_tpm_ovr_cmp')) || ((overallStatus == 'lrn_tpm_ovr_inc'|| overallStatus == 'lrn_tpm_ovr_cln') && RegStatusCode == 'lrn_crs_cmp_cmp')) {
						 		// class details section seemore
			 					var launch_data = {
									...
									pagefrom 		: 'lp_class_seemore'
								};
								launch_data = objectToString(launch_data);
							html += '<tr id="lp_class_seemore_seeless_'+dataId+'" class="lp_class_seemore_seeless'+lastRowInGroup+'"><td class="empty-td"></td><td class="lp-mulcls-separator">';
							html += '<div class="lp_class_seemore" id="lp_class_seemore_'+dataId+'" onclick="$(\'#learningplan-tab-inner\').data(\'contentPlayer\').playContentMylearning(' + launch_data + ');">'+ Drupal.t('LBL713') +'</div>';
							html += '<div class="lp_class_seeless" id="lp_class_seeless_'+dataId+'" onclick="$(\'#learningplan-tab-inner\').data(\'contentPlayer\').lpclassseelesscontent('+dataId+');" style="display:none">'+ Drupal.t('LBL3042') +'</div>';
		  ...
		  html += '<div class ="main-item-container">';
			if(overallStatus == 'lrn_tpm_ovr_enr' || overallStatus == 'lrn_tpm_ovr_inp'  || overallStatus == 'lrn_tpm_ovr_ppm'){
				...
				html += '<span class="enr-statusupdate-user-cls">' + ((userId != updatedBy) ? updatedByName : updatedByName)+" "+Drupal.t("LBL945")+" ";//Viswanthan replaced LBL3046 to datainfo.UpdatedByName #0073510
				...
				if(updatedBy == 0){
					...
					html += '<span class="enr-statusupdate-user-cls">' + ((userId != updatedBy) ? updatedByName : updatedByName)+" "+Drupal.t("LBL945")+" ";//Viswanthan replaced LBL3046 to datainfo.UpdatedName #0073510
				}
				...
				html += '<span class="status-label container">'+ Drupal.t('LBL025') +' : '+'</span>';
				html += '<span class="enr-statusupdate-user-cls">' + ((userId != managerId) ? fullName : fullName)+" "+Drupal.t("LBL945")+" ";//Viswanthan replaced LBL3046 to datainfo.fullName #0073510
				html += '<span class="enr-status-cls">' + regsteredDate + '</span>';
				html += '</div>';
				html += '<div class="item-assigned-code status-row line-item-container">';
		...
		resetFadeOutByClass('.enroll-show-moreclass','item-title-code','line-item-container','MyPrograms'); /*Viswanathan added for #78490*/
		}catch(e){
			// to do
	...
	paintLPClassActions : function(result,prgId,dataAccor){
		...
		var mylearn_content_launchable   	= result['mylearn_content_launchable'];
		var mylearn_survey_pending_flage 	= result['mylearn_survey_pending_flage'];
		var total_content_count 			= result['total_content_count'];
		var content_pending_flag 			= result['content_pending_flag'];
...
//		console.log('result here', result);
					...
					if((baseType == "WBT") && (cntType != null) && (cntType != undefined) && (cntType != '')) {
						var params 		= "{'Id':'"+id+"','enrollId':'"+dataId+"','VersionId':'"+VersionId+"','url1':'"+url1+"','courseId':'"+courseId+"','classId':'"+classId+
						                   "','url2':'"+url2+"','ErrMsg':'"+errmsg+"','contentType':'"+cntType+"','Status':'"+status+
						                     "','LessonLocation':'"+LessonLocation+"','LaunchFrom':'LP','launch_data':'"+launch_data+
		...
		// class details section launch action
		var launch_data = {
				masterEnrollId : master_enrollment_id,
				enrollId :	dataId,
				programId : 0,
				courseId : courseId, 
				classId  : classId,
				...
				defaultContent : '',
			//	classTitle:htmlEntities(classTitle),
				pagefrom : 'lp_class_launch'
			};
			if ((preassessStatus == true || assessStatus == true || mylearn_content_launchable == true)) {
				launch_data = objectToString(launch_data);
				callLaunchUrlFn = 'onclick="$(\'#learningplan-tab-inner\').data(\'contentPlayer\').playContent(' + launch_data + ');"';
			}
	...
	//	console.log('overallStatus:: '+overallStatus+ 'RegStatusCode:: '+RegStatusCode);
		//console.log();
		var inc_can_cmp = 0;
		...
		if(compStatus == "lrn_crs_cmp_inc"){
		  html += '<label class="enroll-launch-full">'+'<span id="lp-class-accodion-'+dataId+'" data="'+dataAccor+'" onclick=\''+objEval+'.addAccordionInClassView(this.className,"0 0","0 -61px","dt-child-row-En",'+objEval+'.getLPClassViewDetails,this,'+objEval+',true);\'></span><a href="javascript:void(0);" class="actionLink enroll-launch clsDisabledButton" >'+Drupal.t("LBL816")+'</a></label>';
			...
			if(RegStatusCode == 'lrn_crs_cmp_enr' || RegStatusCode == 'lrn_crs_cmp_inp' || RegStatusCode == 'lrn_crs_cmp_inc' || RegStatusCode == 'lrn_crs_cmp_cmp'||RegStatusCode == 'lrn_crs_cmp_nsw' ||RegStatusCode == 'lrn_crs_cmp_att'){
		      ...
		      var viewButton = null;
		      var vcjoinButton = false;
		      var launchAvailable = false;
		      var enrollAction = '';
		      var classlaunch = '';
		      var ILTShareAvailable = ((dataDelTypeCode == 'lrn_cls_dty_wbt' || dataDelTypeCode == 'lrn_cls_dty_vod' || ((dataDelTypeCode == 'lrn_cls_dty_ilt' || dataDelTypeCode == 'lrn_cls_dty_vcl') && timeDiffIlt >= 0)) && classStatus == 'lrn_cls_sts_atv' && dedClass != 'Y');
			  var ILTchangeAvailable = ((RegStatusCode == 'lrn_crs_cmp_enr' || RegStatusCode == 'lrn_crs_cmp_nsw') && isSwitchAvail);
		      if (dataDelTypeCode == 'lrn_cls_dty_wbt') { // Show correct button for the WBT class
					  ...
					  if (IsLaunchable || mylearn_content_launchable == true) {
					  	if(hideShare==0){
						  classlaunch = (ILTShareAvailable || ILTchangeAvailable ) ? ' enroll-launch' : ' enroll-launch-full';
						}else{
							 classlaunch = (ILTchangeAvailable) ? ' enroll-launch' : ' enroll-launch-full';
						}						
						  launchAvailable = true;
							var launchLabelVal  = Drupal.t("LBL199");
							if(prevCompStatus != 'notallow'){
								//wbtButtonAction = 'Launch';
                              //classlaunch = (hideShare == 0 && ILTchangeAvailable) ? ' enroll-launch' : ' enroll-launch-full';                            
											...
											"data=\"" + multiContentData + "\" " +
												"value='" + launchLabelVal + "' " +
													"name='" + "Launch" + "' " +
														"id='launch-lp-class-" + dataId + "' " +
														"onclick='" +
									                    callDialogObj + ".callMessageWindow(\"" + escape(classTitle) + "\",\"" + Drupal.t("You need to complete") +" " +escape(prevClassTitle) +" "+ Drupal.t("Course") + "\", "+titleLength+");' >";
							}
						}
							...
							if(prevCompStatus != 'notallow'){
								if (launchInfo.length > 1) {
									html += "<input type='button' class='actionLink "+classlaunch+"' " +
				         			  ...
				         			  html += "<a class='actionLink "+classlaunch+" ctools-modal-ctools-video-style' " +
				         			  "title='" + Drupal.t("LBL199") + "' " +
				         			  "id='launch-lp-class-" + dataId + "' " +
				         			  callLaunchUrlFn +
				         			  " >" +
				                       Drupal.t("LBL199") + "</a>";/*Viswanathan removed use-ajax calss for #79194*/
								}
							}else{
								...
								html += "<a class='actionLink "+classlaunch+" ctools-modal-ctools-video-style' " +
										"title='" + Drupal.t("LBL199") + "' " +
					                    "id='launch-lp-class-" + dataId + "' " +
					                    "onclick='" +callDialogObj + ".callMessageWindow(\"" + escape(classTitle) + "\",\"" + Drupal.t("You need to complete") +" " +prevClassTitle +" "+ Drupal.t("Course") + "\", "+titleLength+");' >" +
				                Drupal.t("LBL199") + "</a>";
							  ...
							  if(mylearn_content_launchable == true && total_content_count>0) {
								  launchAvailable = true;
									...
									if(timeDiffVC <= allowTime && (endTimeDiffVC+afterCompleteAllowTime) >=0){
										html += "<a class='actionLink vtip "+classlaunch+" ' data=\"" + contdata + "\" " +
													"' name='" + SMARTPORTAL.t("Launch") + "' " + " title='"+Drupal.t("LBL880")+"' " +
														" id='launch" + dataId + "' href='javascript:void(0);' " +
																" onclick='window.open(\""+((IsLaunchable) ? callLaunchUrlFn : '')+"\", \"Attachment\", \"width=1000,height=900,toolbar=yes,location=yes,directories=yes,status=yes,menubar=yes,scrollbars=yes,copyhistory=yes,resizable=no\")'>"+ titleRestrictionFadeoutImage(Drupal.t("LBL880"),'exp-sp-mylearning-menulist') + "</a>";
									}else{
										
										var title ,message;
										title =Drupal.t("LBL670");//Session Details
										message = (endTimeDiffVC+afterCompleteAllowTime < 0) ? Drupal.t("MSG419") : Drupal.t("MSG420") +' '+ sessTimeFormat;
										html += "<a class='actionLink vtip "+classlaunch+" ' data=\"" + contdata + "\" " +
						        				"' name='" + SMARTPORTAL.t("Launch") + "' " + " title='"+Drupal.t("LBL880")+"' " +
						        					" id='launch" + dataId + "' href='javascript:void(0);' " +
							  	...
							  	vcjoinButton = true;
							  	classlaunch = ((ILTShareAvailable && hideShare==0) || ILTchangeAvailable || (mylearn_content_launchable == true && total_content_count>0)) ? ' enroll-launch' : ' enroll-launch-full';
								//Launch appears only in the configured time. If not configured allow_meeting_launch then system take default as half hour from session end
							  	if(timeDiffVC <= allowTime && (endTimeDiffVC+afterCompleteAllowTime) >=0){
									html += "<a class='actionLink vtip "+classlaunch+" ' data=\"" + contdata + "\" " +
												"' name='" + SMARTPORTAL.t("Launch") + "' " + " title='"+Drupal.t("LBL880")+"' " +
		      		...
		      		if(!launchAvailable && !vcjoinButton && ILTShareAvailable) {
		      				classlaunch = (ILTchangeAvailable ) ? ' enroll-launch' : ' enroll-launch-full';
                                                if(hideShare == 0){
							...
							'\',\'cre_sys_obt_cls\',\'enroll-lp-result-container\');" >' + titleRestrictionFadeoutImage(Drupal.t("Share"),'exp-sp-mylearning-menulist') + '</a>';
                                            }else if(vcjoinButton==false && ILTchangeAvailable){
                                                html +='<a href="javascript:void(0)" class="actionLink enroll-launch-full vtip" '+
					...
					}else if(!launchAvailable && !vcjoinButton && !ILTShareAvailable && ILTchangeAvailable){
						    html += '<a href="javascript:void(0)" class="actionLink enroll-launch-full vtip" '+
					...
					if((launchAvailable || vcjoinButton || ILTShareAvailable) && ILTchangeAvailable){ //RegStatusCode == 'lrn_crs_cmp_ppm' ||
						if(hideShare == 0 || vcjoinButton){
					...
					if((enrollAction!='' || (ILTShareAvailable && ILTchangeAvailable && hideShare == 0))){
						var linav = true;
		...
		launch_data = objectToString(launch_data);
		if((overallStatus == 'lrn_tpm_ovr_enr' || overallStatus == 'lrn_tpm_ovr_inp' || overallStatus == 'lrn_tpm_ovr_cmp') || ((overallStatus == 'lrn_tpm_ovr_inc'|| overallStatus == 'lrn_tpm_ovr_cln') && RegStatusCode == 'lrn_crs_cmp_cmp')){  // Show progress bar in class level based tp status
			html += '<div class="lpclsprogress progress" id="progress_'+dataId+'"></div>';
			if(RegStatusCode == 'lrn_crs_cmp_cmp') {
				...
				//else
				 if(mylearn_survey_pending_flage == true ){
					//html+='<div class="clsPrgSurveyPendingLable">'+Drupal.t("Take")+' '+ Drupal.t("Survefffy")+'</div>';
					html+='<div class="clsPrgSurveyPendingLable"><a id = "take_survey" onclick="$(\'#learner-enrollment-tab-inner\').data(\'contentPlayer\').launchsurvey('+launch_data+');" >'+Drupal.t("LBL3212")+' '+ Drupal.t("Survey")+'</a></div>';
				}
				}
			if(compStatus != "lrn_crs_cmp_inc"){
			progressBarRound(dataId, progress, 'lp_class_progress','progress_',this);
			//$("#learningplan-tab-inner").data("learningplan").progressbarlp(dataId, progress, 'lp_class_progress');
		}
		}
		html += "</div>";
	...
	getLPSessionDetails : function(sessData,sessionId) {
		try{
		...
		if(!document.getElementById("session_det_lp"+sessionId)) {
			...
			$('#session_det_popup_lp'+sessionId).qtip({
				 content: '<div id="session_det_lp'+sessionId+'" class="qtip_session_top tooltiptop"></div><div class="qtip_session_mid tooltipmid">'+sessionDet1+'</div><div class="qtip_session_bottom tooltipbottom"></div>',
			     show:{
					when:{
						event:'mouseover'
					},
	...
	addAccordionInClassView : function(openCloseClass,openpos,closepos,childClass,callback,obj,parentObj,isRemove) {
		...
		//72790: More and description V should expand all the details instead of only doing what we do today.
		//$('.limit-desc-lp').trunk8(trunk8.myprogramLP_desc);
		resetFadeOutForAttributes('.'+currTr+'LPDetailsTD','main-item-container','line-item-container','container','myprogramclass');
		}catch(e){
			// to do
		}
		vtip();
	...
	paintDetailsLPClassSection : function(data){
		...
		ostr += '<td></td><td class="'+enrollId+'LPDetailsTD"><div id="'+enrollId+'LPDetailsMainDiv" class="enroll-accordian-div">';
...
//			html += pipe+'<span class="vtip" title="'+unescape(classCode).replace(/"/g, '&quot;')+'">'+titleRestrictionFadeoutImage(classCode,'exp-sp-lnrmyprogram-class-code',10)+'</span>';
		...
		if(Drupal.settings.mylearning_right===false){
			/* Viswanathan uncommented for the #0073514*/
			if(classCode)
				ostr += '<div class="traning-plan-cls-titlist line-item-container"><span class="mypgm-group-title container">'+Drupal.t('LBL263')+' : </span><span class="vtip mypgm-group-title-val" title="'+classCode+'">'+titleRestrictionFadeoutImage(classCode,'exp-sp-lnrmyprogram-class-code',50)+'</span></div>';/* Viswanathan uncommented for the #0073514*/
			if(courseTitle)
		...
		if(document.getElementById("session_det_popup_lp"+classId)) {
			...
			$(sessionDet).each(function(){ //sessionDate sessionDay
               var tz_code = $(this).attr("tz_code") ;
				...
				ostr += '<div class="cls-sessionTitle-container line-item-container"><div class="lbl-sessionTitle sessionDetlbl container">'+Drupal.t("LBL107")+' :'+'</div> <div class="sessName vtip" title="'+(($(this).attr("sessionTitle") != '') ? unescape($(this).attr("sessionTitle")).replace(/"/g, '&quot;') : "&nbsp;")+'">'+unescape(sesionsHead)+'</div></div>';
	...
	paintLPLessonSection : function(data,paramContId){
		...
		$(data).each(function(){
          ...
          relaunchInfo += "}";
  		  ...
  		  if(contentQuizStatus != null && contentQuizStatus != undefined && contentQuizStatus != '') {
			  contentQuizStatus = pipe + '<span class="vtip" title="'+ Drupal.t('LBL1284') + ' : ' + Drupal.t(contentQuizStatus) + '">'+ Drupal.t('LBL1284') + ' : ' + Drupal.t(contentQuizStatus) + '</span>';
		  }
  		  if(contScore == '') {
      	    var lessonScore = '';
          } else {
      	    var lessonScore = pipe + Drupal.t('LBL668') + ' : ' + contScore;
          }
				  ostr += '<tr class="enroll-lesson-section"><td class="enroll-launch-column1">';
				  ostr += '<div class="enroll-course-title">';
				  ostr += '<span id="titleAccEn_'+lessonId+'" class="item-title" ><span class="enroll-class-title vtip" title="'+unescape($(this).attr('Title')).replace(/"/g, '&quot;')+'" href="javascript:void(0);">';
				  ...
				  ostr += '<span>'+Drupal.t('LBL036')+' : '+Drupal.t('LBL854')+' </span>';
				  var lessonStatus = ($(this).attr('Status') == '') ? Drupal.t('MSG511') : Drupal.t($(this).attr('Status'));
				  ostr += pipe + Drupal.t('LBL102')+': <span id="' + $(this).attr('Id') + '_' + $(this).attr('ContentId') + '_' + $(this).attr('enrollId') + '_status">'+ lessonStatus +'</span>';
			...
			$(this).each(function(){
				...
				var VersionNo = '';
				  ...
				  if(AvailableScore !=null && AvailableScore !=undefined && AvailableScore != '' && AvailableScore != '0.00') {
						var score = pipe + Drupal.t('LBL668') +' : ' + AvailableScore;
				  }
		        var VersionNo = ($(this).attr('VersionNo') == 'notset')? '' : $(this).attr('VersionNo');
		        if (VersionNo != '') {
		          VersionNo = pipe + Drupal.t('LBL1123')+' : ' + VersionNo;
		        }
		        
		        var attemptsLeft = ($(this).attr('AttemptLeft') == 'notset')? '' : $(this).attr('AttemptLeft');
		        if (attemptsLeft !== '') {
		          attemptsLeft = pipe + Drupal.t('LBL202')+' : ' + attemptsLeft;
		        }
...
//					  var remValidityDays = validityDays - diffDays;
					...
					validityDays = pipe + Drupal.t('LBL604') +' : ' + remValidityDays + ' ' + daysLBL;
				  }
				...
				if(contentType == 'VOD') {
					...
					var progress = suspend_data != null ? suspend_data['progress'] : 0;
					progress = isNaN(parseFloat(progress)) ? 0 : Math.round(progress);
					percentCompleted = pipe + progress+'% '+Drupal.t('Completed');
				}
				if($.inArray(contentId,contentArr) == -1){
			        ...
			        if (contentQuizStatus !== '') {
			        	contentQuizStatusString = pipe + Drupal.t('LBL1284')+' : ' + Drupal.t(contentQuizStatus);
			        }
					var lessonId = $(this).attr('Id');
					var Dstatus = Drupal.t('In progress');
					var Dstatus1 = Drupal.t('Incomplete');
					...
					html +=   titleRestrictionFadeoutImage(unescape(decodeURIComponent($(this).attr('ContentTitle'))),'exp-sp-lnrmyprogram-content-title');
					html +=   '<div class="item-title-code">';
					html +=  Drupal.t('LBL102')+': <span id="lesson_status_'+contentId+'">' + contentStatus + score + contentQuizStatusString + attemptsLeft + validityDays + percentCompleted + VersionNo+'</span>';
					html +=           '</div>';
					html +=         '</span>';
					html +=       '</span>';
					html +=     '</div>';
					...
					if (lcnt == 1 || contentType == 'VOD') {
			            ...
			            relaunchInfo += "}";
			    		var relaunchData = "["+relaunchInfo+"]";
			            var ajaxCss = ((contentType == 'VOD') && (IsLaunchable))? ' ctools-modal-ctools-video-style ' : ' ';/*Viswanathan removed use-ajax calss for #79194*/
	...
	callbackCatalogLoader: function(response, postdata, formid){
	 try{
	  ...
	  vtip();
	  resetFadeOutByClass('#tbl-paintCatalogContentResults','find-training-list-course','line-item-container','changeclass');
