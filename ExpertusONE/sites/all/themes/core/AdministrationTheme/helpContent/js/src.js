...
op7 = ((op && _W.opera.version && _W.opera.version() > 6) || _nu.indexOf("opera 7") != -1 || _nu.indexOf("opera/7") != -1) ? _t : _f;
...
function _StO(_fnc, _ms) {
    return setTimeout(_fnc, _ms)
}
tTipt = Z$;
_m = [];
_mi = [];
...
function f_(_it) {
    _mi[_bl] = [];
    for ($i in _x[6]) if (_x[6][$i]) _mi[_bl][_$S[$i]] = _x[6][$i];
    _mi[_bl][0] = _mn;
    _it = _it.split(";");
    _sc = Z$;
    for (_a = 0; _a < _it.length; _a++) {
        ...
        if (_sp != -1) {
            ...
            _it[_a] = _tI.replace(/`/g, Z$)
        }
        _sp = _it[_a].indexOf("=");
        if (_sp == -1) {
            if (_it[_a]) _si = _si + ";" + _it[_a] + _sc
        } else {
...
function ami(txt) {
    ...
    if (_c == 1) {
        ...
        _S = _m[_mn][6];
        _M = _m[_mn];
        if (_M[15] == _n) _M[15] = _S.overfilter;
        if (_M[16] == _n) _M[16] = _S.outfilter;
        _S[65] = (_S.borderwidth) ? $pU(_S.borderwidth) : 0;
        _S[64] = _S.borderstyle;
        _S[63] = _S.bordercolor;
        if (_W.ignoreCollisions) _M[25] = 1;
        if (!_S.built) {
            _WzI = _zi;
            if (_W.menuZIndex) {
                _WzI = _W.menuZIndex;
            ...
            if (_vC) {
                ...
                if (!_oC) _oC = "#000000";
                if (!_vC) _vC = "#ff0000";
                _Lcl = "<style>.g_" + lcl + ":link{color:" + _oC + "}.g_" + lcl + ":visited{color:" + _vC + "}</style>";
                _d.write(_Lcl);
                _S.g_ = "g_" + lcl
            }
            _S.built = 1
