H5P.InteractiveVideo = (function ($, EventDispatcher, DragNBar, Interaction) {
  ...
  function InteractiveVideo(params, id, contentData) {
    ...
    self.l10n = $.extend({
      interaction: window.parent.Drupal.t('LBL3021'), //'Interaction',
      play: window.parent.Drupal.t('Play'), //'Play',
      pause: window.parent.Drupal.t('LBL3035'), //'Pause',
      mute: window.parent.Drupal.t('Mute'), //'Mute',
      quality: 'Video quality',
      unmute: window.parent.Drupal.t('Unmute'), //'Unmute',
    ...
    self.l10n.play = window.parent.Drupal.t('Play');
    self.l10n.pause = window.parent.Drupal.t('LBL3035');
    self.l10n.mute = window.parent.Drupal.t('Mute');
    self.l10n.unmute = window.parent.Drupal.t('Unmute');
    self.l10n.interaction = window.parent.Drupal.t('LBL3021');
  ...
  InteractiveVideo.prototype.addControls = function () {
    ...
    // Add bookmarks
    //this.addBookmarks();    //commented for removing bookmark - h5pcustomize
  ...
  InteractiveVideo.prototype.loaded = function () {
	...
	if($(".h5p-minimal").size() >  0)
      $(".h5p-video-wrapper").css("margin-top","0px");
  ...
  InteractiveVideo.prototype.attachControls = function ($wrapper) {
    ...
    /*if (!self.editor && H5P.canHasFullScreen !== false && H5P.learnerLaunch == false) 
    {
...
//	}
    ...
    if(H5P.learnerLaunch == false)
    	$(".Fullsc").css("height","564px");
    ...
    self.controls.$slider = $('<div/>', {appendTo: $slider}).slider({
      ...
      stop: function (e, ui) {
        ...
        if (self.lastState === H5P.Video.PLAYING) {
        	  if(ui.value != Math.ceil(self.video.getDuration())) //Added if condition for  0083999: Catalog>>Video is playing automatically when play and scroll it to the end
        		  self.video.play();
        }
        else {
  ...
  // alert("heree : " + self.l10n[type] );
    options[text ? 'text' : 'title'] = window.parent.Drupal.t(self.l10n[type]); //self.l10n[type]; 
  ...
  InteractiveVideo.prototype.resizeControls = function () {
    ...
    if (left || right) {
      ...
      //h5pcustomize for seekbar
      if($(".h5p-interactivevideo-editor").size() > 0)
      {
      	var mR = 115/122*right;
      	this.controls.$slider.parent().css({
        marginLeft: "46px",
        marginRight: mR
      });
  ...
  InteractiveVideo.prototype.timeUpdate = function (time) {
    ...
    if (tenth !== self.lastTenth) {
        //h5pcustomize
		//update slider marginleft dynamically based on the play time
		 var right = this.$controls.children('.h5p-controls-right').width();
    	if($(".h5p-interactivevideo-editor").size() > 0)
      	{
      			var mR = 115/122*right;
