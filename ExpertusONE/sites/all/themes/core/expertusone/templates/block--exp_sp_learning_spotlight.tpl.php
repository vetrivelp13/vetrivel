        ...
        <ul>
          <?php  
            ...
            <li class='list-spotlight<?php print ($num_rows == ($id+1))?'-last':'';?>'>
              ...
              <p class="spotlight-block">
                  <img class="<?php print $icon_img_class;?>" src="<?php print file_create_url(path_to_theme() . '/expertusone-internals/images/spacer.gif');?>" alt="Course Icon" />
                  <?php $curTitle = titleController('BLOCK-SP-LEARNING-SPOTLIGHT-TITLE',$row->title,25); ?>
                  <?php 
                      if($entity_type=="cre_sys_obt_crt" || $entity_type=="cre_sys_obt_cur" || $entity_type=="cre_sys_obt_trn") {
                        $row->node_id =$row->id;
                      }

                        print l($curTitle,$detail_page_url_link.$row->node_id, $options = array('attributes' => array('class' => 'spotlight-item-title vtip', 'title' => $row->title),'html'=>TRUE));
                    ?>
                  <span class="spotlight-title-breaker">&nbsp</span>
           		 <?php print descController($block->blockname,strip_tags($row->short_description)); ?>
              </p>
			</div>
            </li>
           <?php } ?>
