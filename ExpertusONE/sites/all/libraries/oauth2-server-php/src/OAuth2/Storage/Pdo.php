    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    //Added by Vincent on Nov 12, 2014 to get client details by name
    public function getClientDetailsByName($client_name)
    {
        $stmt = $this->db->prepare(sprintf('SELECT * from %s where client_name = :client_name', $this->config['client_table']));
        $stmt->execute(compact('client_name'));

        return $stmt->fetch();
    }
    
    //Added by Ramkumar By Guidance of Vincent on Nov 26, 2014 to get client details by URl and Scope
    public function getClientDetailsByScope($scope,$redirect_uri)
    {
    	$stmt = $this->db->prepare(sprintf('SELECT * from %s where scope = :scope and redirect_uri like :redirect_uri ', $this->config['client_table']));
    	$stmt->execute(compact('scope','redirect_uri'));
    	    
    	return $stmt->fetch();
    }

    // Modified by Vincent on Nov 12, 2014 to add client name as per E1 spec
    public function setClientDetails($client_name, $client_id, $client_secret = null, $redirect_uri = null, $grant_types = null, $scope = null, $user_id = null)
    {
        // if it exists, update it.
        if ($this->getClientDetails($client_id)) {
            $stmt = $this->db->prepare($sql = sprintf('UPDATE %s SET client_name=:client_name, client_secret=:client_secret, redirect_uri=:redirect_uri, grant_types=:grant_types, scope=:scope, user_id=:user_id where client_id=:client_id', $this->config['client_table']));
        } else {
            $stmt = $this->db->prepare(sprintf('INSERT INTO %s (client_name, client_id, client_secret, redirect_uri, grant_types, scope, user_id) VALUES (:client_name, :client_id, :client_secret, :redirect_uri, :grant_types, :scope, :user_id)', $this->config['client_table']));
        }

        return $stmt->execute(compact('client_name','client_id', 'client_secret', 'redirect_uri', 'grant_types', 'scope', 'user_id'));
    }
