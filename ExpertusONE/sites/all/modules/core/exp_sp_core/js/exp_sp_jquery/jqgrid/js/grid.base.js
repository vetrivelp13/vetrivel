...
$.fn.jqGrid = function( p ) {
	...
	return this.each( function() {
		...
		var addCell = function(t,row,cell,pos,rawdata) {
			var td;
			td = document.createElement("td");
			formatter($(td,t),row,cell,pos,'add',rawdata);			
			row.appendChild(td);
			formatCol($(td,t), pos);
		};
		var formatter = function (elem, row, cellval , colpos, act,rawdata){
			var cm = ts.p.colModel[colpos]; 
			if(cm.formatter) {
				var opts= {rowId: row.id, colModel:cm,rowData:row};
				if($.isFunction( cm.formatter ) ) {
					cm.formatter(elem,cellval,opts,rawdata,act);
				} else if($.fmatter){
		...
		var setPager = function (){
			...
			pginp = (ts.p.pginput===true) ? "<span style='float: left'><label class='pgWrapper'><input class='selbox' type='text' size='3' maxlength='5' value='0'/>" : "",
			pgl="", pgr="", str;
			if(ts.p.viewrecords===true) {pginp += "<span id='sp_1'></span></label></span>";}else{pginp += "</label></span>";}
			if(ts.p.pgbuttons===true) {
				//pgl = inpt+" id='first' class='ui-icon-seek-first' /> 123 &#160;&#160;"+inpt+" id='prev' class='ui-icon-seek-prev' />&#160;";
				//pgr = inpt+" id='next' class='ui-icon-seek-next' /> 3456 &#160;&#160;"+inpt+" id='last' class='ui-icon-seek-end'/>";
				pgl = "<span><span id='first' class='ui-icon-seek-first'></span><span id='prev' class='ui-icon-seek-prev'></span></span>";
				pgr = "<span><span id='next' class='ui-icon-seek-next' ></span><span id='last' class='ui-icon-seek-end'></span>";
			}
			var pgTxt = '';
			/* ISSUE #88 | Fixed by - Jaikumar | Date March 04-2011 */
			pgTxt = "<span id='jqPagerDisp' style='float: right; line-height: 18px; font-size: 11px;'><span style='float:right;'>"+pgl+pginp+pgr+"</span>";
			pgrTxt = pgTxt;
			//$(ts.p.pager).append(pgl+pginp+pgr);
			//if(document.getElementById("jqPagerDisp")){
				//$(ts.p.pager).append(pgTxt);
			//}
			if(ts.p.rowList.length >0){
				str="<span style='padding-left: 10px;'><SELECT class='selbox' style='width:50px'>";
				for(var i=0;i<ts.p.rowList.length;i++){
					str +="<option value="+ts.p.rowList[i]+((ts.p.rowNum == ts.p.rowList[i])?' selected':'')+">"+ts.p.rowList[i]+"</option>";
				}
				str +="</SELECT>";
				pgrTxt = pgrTxt+str+"&#160;<span id='sp_2'></span></span>";
				// $(ts.p.pager).append(str+"&#160;<span id='sp_2'></span></span>");				
			} else { 
				//$(ts.p.pager).append("<span id='sp_2'></span>");
				pgrTxt = pgrTxt+"<span id='sp_2'></span>";
			}
			pgrTxt = pgrTxt+"</span>";
			/* ISSUE #88 | Fixed by - Jaikumar | Date March 04-2011 */
			if(!document.getElementById("jqPagerDisp") && !$(".ui-icon-seek-first").attr("id")) {
				$(ts.p.pager).append(pgrTxt);
			}
			if(ts.p.rowList.length >0){
			...
			$("#first, #prev, #next, #last",ts.p.pager).click( function(e) {
				...
				if( this.id === 'next' && np) { ts.p.page=(cp+1); selclick=true; fp = true; pp = true;} 
				if( this.id === 'last' && lp) { ts.p.page=last; selclick=true; fp = true; pp = true;}
				if(selclick) {
		...
		var sortData = function (index, idxcol,reload){			
			...
			if(ts.p.datatype == "local" || ts.p.datatype == 'xmlstring') {
		...
		if(this.p.subGrid) {
			...
			this.p.colModel.unshift({name:'subgrid',width:17,sortable: false,resizable:false,hidedlg:true,search:false});
		}
		if(this.p.multiselect) {
			this.p.colNames.unshift("<input id='cb_jqg' class='cbox' type='checkbox'/>");
			this.p.colModel.unshift({name:'cb',width:22,sortable:false,resizable:false,hidedlg:true,search:false});
		}
		var	xReader = {
			root: "rows",
			row: "row",
		...
		$(ts).before(grid.hDiv).css("width", grid.width+"px").click(function(e) {
			...
			if(($(t).attr("id")==$(ts).attr("id")) || t==undefined)
				populate();
		});	
		if( ondblClickRow ) {
		...
		$(grid.bDiv)
			...
			.css({ height: "auto", padding: "0px", margin: "0px", overflow: ofl2,width: "100%"} ).css("overflow-x","hidden")
			.append(this);
