...
abstract class DrupalTestCase {
  ...
  public function __construct($test_id = NULL) {
	...
	/**
   * Set http referer value
   * 
   * @param $value
   * url need to set in the refrere this should exclued the base url
   */
  protected function setHttpReferer($string){
  	global $base_url;
  	if(!empty($string)){
  		$this->http_referer = $base_url . "/?q=" . $string;
  	}else{
  		$this->http_referer = $base_url;
  	}
  }
  /**
...
class DrupalWebTestCase extends DrupalTestCase {
  ...
  protected function curlExec($curl_options, $redirect = FALSE) {
    ...
    //CURLOPT_REFERER added to the CURL options. 
    // Added by Vincent
    if(!empty($this->http_referer)){
    	$curl_options[CURLOPT_REFERER] = $this->http_referer;
    }
    curl_setopt_array($this->curlHandle, $this->additionalCurlOptions + $curl_options);
      ...
      foreach ($forms as $form) {
        ...
        if (!$edit && ($submit_matches || !isset($submit))) {
          ...
          if ($upload) {
            ...
            // Expertus File upload override - Thamil
            foreach ($upload as $key => $file) {
              $file = drupal_realpath($file);
              if ($file && is_file($file)) {
              	//$post[$key] = '@' . $file;
              	$fileName = basename($file);
                $post[$key] = getCurlValue($file, $fileName);
              }
            }
            $extra_post_temp = explode("&", $extra_post);
            foreach($extra_post_temp as $key => $value) {
            	$post_key =  explode("=", $value);	
            	$extra_post_array[urldecode($post_key[0])] = urldecode($post_key[1]);
            }
            $post =  array_merge($post, $extra_post_array); // combine two array into single array.
          }
          else {
      ...
      foreach ($return as $command) {
        switch ($command['command']) {
          ...
          case 'insert':
            ...
            if (!isset($command['selector']) || empty($command['selector'])) {
            ...
            }else{ // if selector given
            	$xpath = new DOMXPath($dom);
            	$selectors = array_filter(explode('#',trim($command['selector'])));
            	$selector = '';
            	foreach($selectors as $select){
            		$selector .= '//*[@id="' . trim($select) . '"]';
            	}
            	if(!empty($selector)){
	            	$wrapperNode = $xpath->query($selector)->item(0);
	            	if(!$wrapperNode){
	            		$wrapperNode = $xpath->query(str_replace('_','-',$selector))->item(0);
	            	}
	            	if($wrapperNode){
		            	$newDom = new DOMDocument();
			            $newDom->loadHTML('<div>' . str_replace("'","\'",$command['data']) . '</div>');
			            $newNode = $dom->importNode($newDom->documentElement->firstChild->firstChild, TRUE);
			            $method = isset($command['method']) ? $command['method'] : $ajax_settings['method'];
		            	switch ($method) {
	                  case 'replaceWith':
          ...
          case 'restripe':
            break;
          case 'modal_display': // Added by Vincent
          	$xpath = new DOMXPath($dom);
            $wrapperNode = $xpath->query('//*[@id="modal-content"]')->item(0);
            if($wrapperNode){
            	$newDom = new DOMDocument();
	            $newDom->loadHTML('<div>' . $command['output'] . '</div>');
	            $newNode = $dom->importNode($newDom->documentElement->firstChild->firstChild, TRUE);
	            $wrapperNode->appendChild($newNode);
            }
          	break;
      ...
      // Added by Vincent
        ...
        switch ($k){
        	case 'drupal_settings' :
        		$drupal_settings = array_merge_recursive($drupal_settings, $v);
            break;
        	case 'render_content_main':
        		$xpath = new DOMXPath($dom);
            $wrapperNode = $xpath->query('//*[@id="' . $ajax_settings['wrapper'] . '"]')->item(0);
            if ($wrapperNode) {
              $newDom = new DOMDocument();
	            $newDom->loadHTML('<div>' . $v . '</div>');
	            $newNode = $dom->importNode($newDom->documentElement->firstChild->firstChild->firstChild, TRUE);
	            $wrapperNode->appendChild($newNode);
	            //$wrapperNode->parentNode->replaceChild($newNode, $wrapperNode);
            } 
        }
...
function getCurlValue($filename, $postname)
{
