...
function parseResponse($result) {
	// 	expDebug::dPrint('$result '.print_r($result, 1));
	$return = '<ul>';
	foreach ($result as $key=>$value) {
		if(is_array($value)) {
			...
			$return .= '<li>'.$key.'-'.$description[0].'</li>';
		}
	}
	return $return.'</ul>';
}
...
<div id="content-outer">
	...
	foreach($DE->doc as $key => $val){
		...
		if($key != 'documentation'){
				if( strtolower(trim($val['category']))=="managerapi" && trim($val['customer_display']) == true) {
			?>
			...
			<div class="toc" id="toc">
				...
				<div class="toc-list">
					<ol>
						<li class="toc-level-1"><a href="#otherres">List of APIs for Manager
								functionality</a>
							<ol>
								<?php } ?>
...
<div class="section"><!-- block start --> <?php foreach($DE->doc as $key => $val){
	...
	$responsedet = array();
		...
		{
			if( strtolower(trim($val['category']))!="managerapi") {
				continue;
			}
		}
