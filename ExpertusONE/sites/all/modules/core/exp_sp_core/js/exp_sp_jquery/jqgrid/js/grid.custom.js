...
**/ 
$.fn.extend({
	showPreviousPage: function(widgetNm) {		
		$t = this[0];		
		if ( !$t.grid ) {  return; }
		if($t.p.pagination.currentPage == 1){return;}
		...
		if(widgetNm != undefined && widgetNm != ''){
			var widgetObj = eval(widgetNm);
			if(widgetObj != undefined && widgetObj != null && widgetObj.searchPageNo != undefined){
				widgetObj.searchPageNo = $t.p.pagination.currentPage;
			}
			//Call back function addedd for data table navigation by Gaja on Mar 9th 2011
			var widgetGlobalInstance = eval(widgetObj.widgetGlobalName);
			if(typeof (widgetGlobalInstance.dataTablePaginationCallBack)=='function'){
				widgetGlobalInstance.dataTablePaginationCallBack();
			}
		}
	},
	showNextPage: function(widgetNm) {     
		$t = this[0];
		if ( !$t.grid ) { return; }		
		if($t.p.pagination.currentPage >= $t.p.pagination.totalPages){ return;}
		$t.p.pagination.startIndex = $t.p.pagination.startIndex + $t.p.pagination.maxRecordsPerPage;			
		$($t).trigger('reloadGrid',$t);
		$('.ItemStats').each(function(){
			var nextObj = $(this).next('.ItemStatsDiv:first');
	...
	showFirstPage: function(widgetNm) {		
		$t = this[0];		
		if ( !$t.grid ) { return; }
		if($t.p.pagination.currentPage == 1){ return;}
		$t.p.pagination.startIndex = 0;		
	...
	showLastPage: function(widgetNm) {		
		$t = this[0];		
		if ( !$t.grid ) { return; }		
		if($t.p.pagination.currentPage == $t.p.pagination.totalPages){ return;}
		$t.p.pagination.startIndex = $t.p.pagination.maxRecordsPerPage * ($t.p.pagination.totalPages - 1);
		$($t).trigger('reloadGrid',$t);
		$('.ItemStats').each(function(){
			var nextObj = $(this).next('.ItemStatsDiv:first');
			var obj = $(this);
	...
	showPage: function(pageNo) {	
		$t = this[0];		
		if ( !$t.grid ) { return; }		
		if(pageNo < 0 || pageNo > $t.p.pagination.totalPages){
			return;
		} else {
			$t.p.pagination.currentPage = pageNo;
			if(pageNo == 0){
				$t.p.pagination.startIndex = 0;
			} else {
				$t.p.pagination.startIndex = $t.p.pagination.maxRecordsPerPage * (pageNo - 1);
			}			
	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			$.each($t.p.colModel,function(i,n){
				...
				thd = $("<div style='position:relative;height:100%;padding-right:0.3em;text-align:left;padding-left:1px;'></div>");
				...
				if(this.search){
					...
					case 'text':
						...
						$(thd).append("<input type='text' style='width:75%;padding:0px;' name='"+(cm.index || cm.name)+"' id='gs_"+cm.name+"' value='"+df+"'/>");
