...
(function($) {
	...
	$.tree = {
		...
		defaults	: {
			...
			lang : {
				new_node	: "New profile", //"New folder"
				loading		: "Loading ..."
			},
			callback	: {
				...
				if(this.settings.ui.theme_name !== false) {
					if(this.settings.ui.theme_path === false) {
						$("script").each(function () {
							/*
							 * Tweak: 	theme path has been hardcoded
							 */
							//alert(this.src);
							...
							_this.settings.ui.theme_path = themepath +"/tree/" + _this.settings.ui.theme_name + "/style.css";
							return false;
						});
					}
	...
	$(function () {
		...
		/*
		 * tree li a line-height changed from 16px to 20px - UNDONE
		 * .tree ul ins width: 16px to 12px, height: 16px to 12px
		 */
